...
// RUN: %clang_cc1 -fopenmp -fopenmp-target-fast -fopenmp-assume-teams-oversubscription -fopenmp-assume-threads-oversubscription -fopenmp-assume-no-nested-parallelism -fopenmp-assume-no-thread-state -fno-openmp-target-big-jump-loop -x c++ -std=c++11 -fopenmp-version=50 -triple x86_64-unknown-unknown -fopenmp-targets=amdgcn-amd-amdhsa -emit-llvm-bc %s -o %t-ppc-host.bc
