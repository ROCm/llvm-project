...
void AMDGPUToolChain::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  // Offload code targeting AMDGPU frequently includes ROCm/HIP headers
...
void AMDGPUToolChain::AddFlangSystemIncludeArgs(
    const ArgList &DriverArgs, ArgStringList &Flang1Args) const {
  if (DriverArgs.hasArg(options::OPT_nostdinc))
    return;

  // Add the include directory from the install directory.
  SmallString<128> P(getDriver().Dir);
  llvm::sys::path::append(P, "../include");
  Flang1Args.push_back("-stdinc");
  Flang1Args.push_back(DriverArgs.MakeArgString(P));
}
