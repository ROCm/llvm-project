...
private:
  ...
  /// emitted only for HIP host compiles when PGO is on AND the host is Windows
  /// (the Windows device profile drain uses host shadows; Linux uses HSA
  /// introspection and emits no shadow). Registered via __hipRegisterVar
  /// (non-RDC) or an offloading entry (RDC) so the runtime can locate the
  ...
  /// For HIP+PGO targeting a Windows host, emit the per-TU
  /// __llvm_profile_sections_<CUID> global. On the device side it is the
  /// populated 7-pointer section-bounds table. On the host side it is a
  /// placeholder void* shadow stored in OffloadProfShadow, registered later by
  /// makeRegisterGlobalsFn (non-RDC) or createOffloadingEntries (RDC) so the
  /// runtime can locate the device-side table by name. No-op on non-Windows
  /// hosts, which drain via HSA introspection with no host shadow.
...
llvm::Function *CGNVCUDARuntime::makeRegisterGlobalsFn() {
  ...
  // locate the matching device-side __llvm_profile_sections_<CUID>. Only
  // populated on Windows hosts (see emitOffloadProfilingSections); on other
  // hosts OffloadProfShadow stays null and this block is a no-op. We emit
  // both __hipRegisterVar (so the HIP runtime can map the host shadow to the
  // device symbol) and __llvm_profile_offload_register_shadow_variable (so the
  // profile runtime adds the shadow to its drain list).
...
void CGNVCUDARuntime::createOffloadingEntries() {
  ...
  // Register the per-TU offload-profiling shadow (Windows hosts only; null
  // elsewhere). The offloading entry makes the linker-wrapper emit the host
  // __hipRegisterVar call in the combined ctor. Separately emit a per-TU ctor
  // that registers the shadow with the profile runtime's drain list.
  if (OffloadProfShadow) {
    llvm::offloading::emitOffloadingEntry(
...
// For HIP host+device compiles with PGO enabled and a Windows host, emit the
// per-TU global __llvm_profile_sections_<CUID>. Device side: a 7-pointer
// struct holding section start/stop bounds for the names/counters/data
// sections plus the raw-version variable. Host side: an opaque void* shadow
// whose only purpose is to give the host-runtime a registered symbol name to
// look up via hipGetSymbolAddress; the actual device-side data lives in the
// matching device-side global.
//
// This is the single platform gate for the legacy host-shadow drain: on
// non-Windows hosts we return immediately and emit nothing, so all the
// shadow-registration blocks above (guarded by OffloadProfShadow) become
// no-ops and the Linux HSA-introspection drain is used instead.
void CGNVCUDARuntime::emitOffloadProfilingSections() {
  ...
  // Host-shadow drain is Windows-only. Determine the *host* triple: on a device
  // compile the host is the aux target; on a host compile it is the target.
  const llvm::Triple *HostTriple = nullptr;
  if (CGM.getLangOpts().CUDAIsDevice) {
    if (const TargetInfo *Aux = CGM.getContext().getAuxTargetInfo())
      HostTriple = &Aux->getTriple();
  } else {
    HostTriple = &CGM.getTarget().getTriple();
  }
  if (!HostTriple || !HostTriple->isOSWindows())
    return;
