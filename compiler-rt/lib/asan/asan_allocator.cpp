...
hsa_status_t asan_hsa_amd_agents_allow_access(
  ...
  BufferedStackTrace *stack) {
  (void)stack;
  void *const block_begin = get_allocator().GetBlockBegin(ptr);
  const void *const actual_ptr = block_begin ? block_begin : ptr;
  ...
  // For ASan-managed allocations, also allow peer GPUs to access the shadow
  // range for this block so P2P + instrumented access does not fault on shadow
  // checks (ROCm llvm-project PR #1992, ROCM-20344). Failures on the shadow grant
  // are ignored when shadow lives in already-accessible system memory.
  if (block_begin) {
    const uptr alloc_size = static_cast<uptr>(
        get_allocator().GetActuallyAllocatedSize(block_begin));
    if (alloc_size > 0) {
      const uptr data_begin = reinterpret_cast<uptr>(block_begin);
      const uptr data_end = data_begin + alloc_size;
      const uptr aligned_begin = RoundDownTo(data_begin, ASAN_SHADOW_GRANULARITY);
      const uptr aligned_end = RoundUpTo(data_end, ASAN_SHADOW_GRANULARITY);
      const uptr shadow_begin = MEM_TO_SHADOW(aligned_begin);
      const uptr shadow_end = MEM_TO_SHADOW(aligned_end);
      if (shadow_end > shadow_begin) {
        (void)REAL(hsa_amd_agents_allow_access)(
            num_agents, agents, flags,
            reinterpret_cast<void *>(shadow_begin));
      }
    }
  }
