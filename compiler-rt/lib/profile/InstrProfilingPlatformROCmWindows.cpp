//===- InstrProfilingPlatformROCmWindows.cpp - ROCm platform (Windows) ---===//
//
...
//===----------------------------------------------------------------------===//
//
// Windows host-side ROCm/HIP device profile collection (legacy host-shadow +
// hipMemcpy drain). Windows has no HSA introspection, so it keeps this method;
// Linux uses HSA introspection in InstrProfilingPlatformROCm.cpp instead. The
// two files are mutually exclusive by platform guard so exactly one provides
// the __llvm_profile_offload_* / __llvm_profile_hip_collect_device_data symbols.
//
//===----------------------------------------------------------------------===//
