//===- InstrProfilingPlatformROCmWindows.cpp - ROCm profile (Windows) ----===//
//
...
//===----------------------------------------------------------------------===//
//
// Windows-only device profile drain for AMDGPU.
//
// Windows ships HIP (amdhip64.dll) but no HSA runtime, so the introspection-
// based drain in InstrProfilingPlatformROCm.cpp (which walks HSA code objects)
// cannot run here. This file keeps the legacy host-shadow mechanism instead:
// the compiler emits a per-TU host shadow for each device-side
// __llvm_profile_sections_<CUID> table (see CGCUDANV.cpp, gated to Windows
// hosts) and registers it with the HIP runtime; at the host profile-write /
// process-exit drain we resolve each shadow with hipGetSymbolAddress and copy
// the device counters back with hipMemcpy.
//
// This path therefore requires the host translation units to be instrumented
// (the shadow registration lives in host code). Decoupled, host-uninstrumented
// collection is a Linux-only feature and is intentionally not supported here.
//
// The CMake (compiler-rt/lib/profile/CMakeLists.txt) compiles exactly one of
// this file or InstrProfilingPlatformROCm.cpp, selected by host platform.
//
//===----------------------------------------------------------------------===//

// Belt-and-suspenders: CMake only adds this file on WIN32, but guard the body
// so an accidental non-Windows compile reduces to an empty TU rather than
// colliding with the Linux drain's entry points.
#if defined(_WIN32)
