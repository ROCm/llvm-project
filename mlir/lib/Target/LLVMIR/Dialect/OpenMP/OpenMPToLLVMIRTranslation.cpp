...
static void collectMapDataFromMapOperands(
  ...
  for (Value mapValue : hasDevAddrOperands) {
    ...
    mapData.MapClause.push_back(mapOp.getOperation());
    if (llvm::to_underlying(mapType & mapTypeDescriptor)) {
      // Descriptors can get rematerialized, so don't use the address
      // of the decriptor, just copy the descriptor.
...
mapParentWithMembers(LLVM::ModuleTranslation &moduleTranslation,
  ...
  if (!parentClause.getPartialMap()) {
    ...
    mapFlag &= ~llvm::omp::OpenMPOffloadMappingFlags::OMP_MAP_DESCRIPTOR;
