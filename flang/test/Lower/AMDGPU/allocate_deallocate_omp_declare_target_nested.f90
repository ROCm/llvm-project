! RUN: %flang -ffast-amd-memory-allocator -S -emit-llvm -fopenmp -fopenmp-version=52 -fopenmp-targets=amdgcn-amd-amdhsa --offload-arch=gfx90a -o - %s | FileCheck %s --check-prefix=CHECK-OMP
...
! CHECK: call void @_FortranAAMDAllocatableSetAllocIdx({{.*}}, i32 5)
