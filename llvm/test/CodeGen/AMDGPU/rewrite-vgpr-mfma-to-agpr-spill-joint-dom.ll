; REQUIRES: asserts
; RUN: llc -O3 -amdgpu-late-wave-transform=0 -mtriple=amdgpu9.50-amd-amdhsa \
; RUN:   -stop-after=amdgpu-rewrite-agpr-copy-mfma \
; RUN:   -debug-only=amdgpu-rewrite-agpr-copy-mfma -filetype=null %s 2>&1 \
; RUN:   | FileCheck %s

; TODO-WAVETRANSFORM: With LWT=1, register allocation changes such that the
; non-jointly-dominated spill reload scenario does not manifest. Force LWT=0
; to preserve the test's original intention. Should revisit this test later.
