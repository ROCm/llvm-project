...
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -stress-regalloc=1 --regalloc-npm='default,default,greedy<vgpr>' -filetype=null %s 2>&1 | FileCheck -check-prefixes=CHECK,GREEDY -implicit-check-not=error %s

; FIXME: test this with new PM when basic regalloc is ported
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -stress-regalloc=1 -enable-new-pm=0 -vgpr-regalloc=basic -filetype=null %s 2>&1 | FileCheck -implicit-check-not=error -check-prefixes=CHECK,BASIC %s

; RUN: not llc -mtriple=amdgcn-amd-amdhsa -stress-regalloc=1 --regalloc-npm='default,default,regallocfast<filter=vgpr>' -filetype=null %s 2>&1 | FileCheck -implicit-check-not=error -check-prefixes=CHECK,FAST %s
; RUN: opt -passes=debugify -o %t.bc %s
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -stress-regalloc=1 --regalloc-npm='default,default,greedy<vgpr>' -filetype=null %t.bc 2>&1 | FileCheck -implicit-check-not=error -check-prefixes=DBGINFO-CHECK,DBGINFO-GREEDY %s

; FIXME: test this with new PM when basic regalloc is ported
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -stress-regalloc=1 -enable-new-pm=0 -vgpr-regalloc=basic -filetype=null %t.bc 2>&1 | FileCheck -implicit-check-not=error -check-prefixes=DBGINFO-CHECK,DBGINFO-BASIC %s
