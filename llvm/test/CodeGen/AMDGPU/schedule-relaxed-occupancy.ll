; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.06  < %s | FileCheck --check-prefix=OCC %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.06 -amdgpu-use-amdgpu-trackers=1  < %s | FileCheck --check-prefix=OCC-GCNTRACKER %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.06 -amdgpu-schedule-relaxed-occupancy=true  < %s | FileCheck --check-prefix=RELAX %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.06 -amdgpu-use-amdgpu-trackers=1 -amdgpu-schedule-relaxed-occupancy=true  < %s | FileCheck --check-prefix=RELAX-GCNTRACKER %s
