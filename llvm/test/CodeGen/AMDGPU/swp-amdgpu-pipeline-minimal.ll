...
; RUN: llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx950 --verify-machineinstrs %s -o - | FileCheck %s --check-prefixes=CHECK,CHECK-NOT-PIPELINED
; RUN: llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx950 -amdgpu-enable-pipeliner --verify-machineinstrs %s -o - | FileCheck %s --check-prefixes=CHECK,CHECK-PIPELINED
...
; CHECK-NOT-PIPELINED-LABEL: swp_amdgpu_pipeline_minimal:
; CHECK-NOT-PIPELINED:       ; %bb.0: ; %bb
; CHECK-NOT-PIPELINED-NEXT:    s_load_dword s0, s[4:5], 0x0
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s4, 0
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v1, 0
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v0, 0
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v3, 0
; CHECK-NOT-PIPELINED-NEXT:    s_waitcnt lgkmcnt(0)
; CHECK-NOT-PIPELINED-NEXT:    s_bitcmp1_b32 s0, 0
; CHECK-NOT-PIPELINED-NEXT:    s_cselect_b64 vcc, -1, 0
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v2, 0
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s5, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s6, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s7, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s2, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s9, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s3, s4
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s8, s4
; CHECK-NOT-PIPELINED-NEXT:  .LBB0_1: ; %bb1
; CHECK-NOT-PIPELINED-NEXT:    ; =>This Inner Loop Header: Depth=1
; CHECK-NOT-PIPELINED-NEXT:    s_cmp_lt_i32 s9, 1
; CHECK-NOT-PIPELINED-NEXT:    s_cselect_b64 s[0:1], -1, 0
; CHECK-NOT-PIPELINED-NEXT:    s_or_b32 s9, s3, 1
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v4, s2
; CHECK-NOT-PIPELINED-NEXT:    v_mov_b32_e32 v5, s9
; CHECK-NOT-PIPELINED-NEXT:    buffer_load_dword v4, v4, s[4:7], 0 offen
; CHECK-NOT-PIPELINED-NEXT:    s_and_b64 s[0:1], vcc, s[0:1]
; CHECK-NOT-PIPELINED-NEXT:    buffer_load_dword v5, v5, s[4:7], 0 offen
; CHECK-NOT-PIPELINED-NEXT:    s_add_i32 s3, s3, 1
; CHECK-NOT-PIPELINED-NEXT:    s_add_i32 s2, s2, 2
; CHECK-NOT-PIPELINED-NEXT:    v_pk_add_f32 v[2:3], v[2:3], 1.0 op_sel_hi:[1,0]
; CHECK-NOT-PIPELINED-NEXT:    s_movk_i32 s9, 0x100
; CHECK-NOT-PIPELINED-NEXT:    s_cmp_lg_u32 s8, 0
; CHECK-NOT-PIPELINED-NEXT:    s_mov_b32 s8, 1
; CHECK-NOT-PIPELINED-NEXT:    s_waitcnt vmcnt(1)
; CHECK-NOT-PIPELINED-NEXT:    v_cndmask_b32_e64 v4, 0, v4, s[0:1]
; CHECK-NOT-PIPELINED-NEXT:    s_waitcnt vmcnt(0)
; CHECK-NOT-PIPELINED-NEXT:    v_cndmask_b32_e32 v5, 0, v5, vcc
; CHECK-NOT-PIPELINED-NEXT:    v_pk_add_f32 v[0:1], v[0:1], v[4:5]
; CHECK-NOT-PIPELINED-NEXT:    s_cbranch_scc1 .LBB0_1
; CHECK-NOT-PIPELINED-NEXT:  ; %bb.2: ; %bb12
; CHECK-NOT-PIPELINED-NEXT:    s_endpgm
;
