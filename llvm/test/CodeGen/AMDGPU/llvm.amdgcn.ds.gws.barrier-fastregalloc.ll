...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-mesa-mesa3d -O0 -stop-after=postrapseudos -o - < %s | FileCheck -enable-var-scope -check-prefix=MIR %s
...
define amdgpu_kernel void @gws_barrier_offset0(i32 %val) #0 {
  ...
  ; MIR-NEXT:   BUNDLE implicit killed $vgpr0, implicit $m0, implicit $exec :: (load (s32) from custom "GWSResource") {
  ; MIR-NEXT:     DS_GWS_BARRIER $vgpr0, 0, implicit $m0, implicit $exec :: (load (s32) from custom "GWSResource")
