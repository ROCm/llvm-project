; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py UTC_ARGS: --version 4
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel=0 -mtriple=amdgpu9.00-amd-amdhsa < %s | FileCheck -check-prefixes=GCN,SDAG %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel=1 -global-isel-abort=2 -mtriple=amdgpu9.00-amd-amdhsa < %s | FileCheck -check-prefixes=GCN,GISEL %s

define i128 @fptosi_f64_to_i128(double %x) {
; SDAG-LABEL: fptosi_f64_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_mov_b32_e32 v5, v1
; SDAG-NEXT:    v_bfe_u32 v6, v5, 20, 11
; SDAG-NEXT:    v_mov_b32_e32 v7, 0
; SDAG-NEXT:    s_mov_b64 s[4:5], 0x3ff
; SDAG-NEXT:    v_cmp_gt_u64_e64 s[6:7], s[4:5], v[6:7]
; SDAG-NEXT:    v_mov_b32_e32 v4, v0
; SDAG-NEXT:    v_mov_b32_e32 v0, 0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB0_5
; SDAG-NEXT:  .LBB0_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    s_mov_b64 s[4:5], 0x432
; SDAG-NEXT:    v_ashrrev_i32_e32 v8, 31, v5
; SDAG-NEXT:    v_cmp_lt_u64_e32 vcc, s[4:5], v[6:7]
; SDAG-NEXT:    v_and_b32_e32 v0, 0xfffff, v5
; SDAG-NEXT:    v_or_b32_e32 v3, 1, v8
; SDAG-NEXT:    v_or_b32_e32 v5, 0x100000, v0
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB0_3
; SDAG-NEXT:  .LBB0_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u32_e32 v0, 0x433, v6
; SDAG-NEXT:    v_lshrrev_b64 v[4:5], v0, v[4:5]
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v4, v3, 0
; SDAG-NEXT:    v_mad_u64_u32 v[6:7], s[4:5], v5, v3, v[1:2]
; SDAG-NEXT:    v_mov_b32_e32 v1, v6
; SDAG-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v4, v8, v[1:2]
; SDAG-NEXT:    v_add_co_u32_e64 v2, s[4:5], v7, v2
; SDAG-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, s[4:5]
; SDAG-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v5, v8, v[2:3]
; SDAG-NEXT:    v_mul_lo_u32 v5, v8, v5
; SDAG-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v8, v4, v[2:3]
; SDAG-NEXT:    v_mul_lo_u32 v4, v8, v4
; SDAG-NEXT:    v_add3_u32 v3, v4, v3, v5
; SDAG-NEXT:  .LBB0_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[4:5], exec, vcc
; SDAG-NEXT:    s_and_b64 s[4:5], s[4:5], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[4:5]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB0_5
; SDAG-NEXT:  .LBB0_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_add_u32_e32 v9, 0xfffffbcd, v6
; SDAG-NEXT:    v_sub_u32_e32 v0, 0x473, v6
; SDAG-NEXT:    v_add_u32_e32 v2, 0xfffffb8d, v6
; SDAG-NEXT:    v_lshrrev_b64 v[0:1], v0, v[4:5]
; SDAG-NEXT:    v_lshlrev_b64 v[6:7], v2, v[4:5]
; SDAG-NEXT:    v_lshlrev_b64 v[4:5], v9, v[4:5]
; SDAG-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v9
; SDAG-NEXT:    v_cndmask_b32_e32 v1, v7, v1, vcc
; SDAG-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v9
; SDAG-NEXT:    v_cndmask_b32_e32 v0, v6, v0, vcc
; SDAG-NEXT:    v_cndmask_b32_e32 v9, 0, v4, vcc
; SDAG-NEXT:    v_cndmask_b32_e64 v7, 0, v1, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e64 v6, 0, v0, s[4:5]
; SDAG-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v9, v3, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_cndmask_b32_e32 v12, 0, v5, vcc
; SDAG-NEXT:    v_mul_lo_u32 v10, v8, v6
; SDAG-NEXT:    v_mul_lo_u32 v11, v3, v7
; SDAG-NEXT:    v_mad_u64_u32 v[6:7], s[4:5], v3, v6, 0
; SDAG-NEXT:    v_mad_u64_u32 v[3:4], s[4:5], v12, v3, v[1:2]
; SDAG-NEXT:    v_add3_u32 v7, v7, v11, v10
; SDAG-NEXT:    v_mov_b32_e32 v1, v3
; SDAG-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v9, v8, v[1:2]
; SDAG-NEXT:    v_mad_u64_u32 v[5:6], s[4:5], v8, v9, v[6:7]
; SDAG-NEXT:    v_mul_lo_u32 v7, v8, v12
; SDAG-NEXT:    v_mul_lo_u32 v9, v8, v9
; SDAG-NEXT:    v_add_co_u32_e32 v2, vcc, v4, v2
; SDAG-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, vcc
; SDAG-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v12, v8, v[2:3]
; SDAG-NEXT:    v_add3_u32 v4, v9, v6, v7
; SDAG-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v5
; SDAG-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; SDAG-NEXT:  .LBB0_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptosi_f64_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v5, v1
; GISEL-NEXT:    v_bfe_u32 v6, v5, 20, 11
; GISEL-NEXT:    v_mov_b32_e32 v7, 0
; GISEL-NEXT:    s_mov_b64 s[4:5], 0x3fe
; GISEL-NEXT:    v_cmp_lt_u64_e32 vcc, s[4:5], v[6:7]
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    v_mov_b32_e32 v0, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB0_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    s_mov_b64 s[4:5], 0x432
; GISEL-NEXT:    v_ashrrev_i32_e32 v8, 31, v5
; GISEL-NEXT:    v_and_b32_e32 v0, 0xfffff, v5
; GISEL-NEXT:    v_cmp_lt_u64_e32 vcc, s[4:5], v[6:7]
; GISEL-NEXT:    v_or_b32_e32 v9, 1, v8
; GISEL-NEXT:    v_or_b32_e32 v5, 0x100000, v0
; GISEL-NEXT:    ; implicit-def: $vgpr0_vgpr1
; GISEL-NEXT:    ; implicit-def: $vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[4:5], vcc
; GISEL-NEXT:    s_xor_b64 s[8:9], exec, s[4:5]
; GISEL-NEXT:    s_cbranch_execz .LBB0_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_add_u32_e32 v3, 0xfffffbcd, v6
; GISEL-NEXT:    v_sub_u32_e32 v0, 0x473, v6
; GISEL-NEXT:    v_add_u32_e32 v2, 0xfffffb8d, v6
; GISEL-NEXT:    v_lshrrev_b64 v[0:1], v0, v[4:5]
; GISEL-NEXT:    v_lshlrev_b64 v[6:7], v2, v[4:5]
; GISEL-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v3
; GISEL-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v3
; GISEL-NEXT:    v_lshlrev_b64 v[3:4], v3, v[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v1, v7, v1, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v0, v6, v0, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v10, 0, v3, vcc
; GISEL-NEXT:    v_cndmask_b32_e64 v7, 0, v1, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e64 v5, 0, v0, s[4:5]
; GISEL-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v10, v9, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_cndmask_b32_e32 v12, 0, v4, vcc
; GISEL-NEXT:    v_mul_lo_u32 v11, v8, v5
; GISEL-NEXT:    v_mad_u64_u32 v[3:4], s[4:5], v12, v9, v[1:2]
; GISEL-NEXT:    v_mul_lo_u32 v7, v9, v7
; GISEL-NEXT:    v_mad_u64_u32 v[5:6], s[4:5], v9, v5, 0
; GISEL-NEXT:    v_mul_lo_u32 v9, v8, v10
; GISEL-NEXT:    v_mov_b32_e32 v1, v3
; GISEL-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v10, v8, v[1:2]
; GISEL-NEXT:    v_add3_u32 v6, v6, v7, v11
; GISEL-NEXT:    v_mad_u64_u32 v[5:6], s[4:5], v8, v10, v[5:6]
; GISEL-NEXT:    v_mul_lo_u32 v7, v8, v12
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v4, v2
; GISEL-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, vcc
; GISEL-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v12, v8, v[2:3]
; GISEL-NEXT:    v_add3_u32 v4, v9, v6, v7
; GISEL-NEXT:    ; implicit-def: $vgpr6_vgpr7
; GISEL-NEXT:    ; implicit-def: $vgpr9
; GISEL-NEXT:    ; implicit-def: $vgpr8
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v5
; GISEL-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr4_vgpr5
; GISEL-NEXT:  .LBB0_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[4:5], s[8:9]
; GISEL-NEXT:    s_cbranch_execz .LBB0_5
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_u32_e32 v0, 0x433, v6
; GISEL-NEXT:    v_lshrrev_b64 v[4:5], v0, v[4:5]
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mad_u64_u32 v[0:1], s[8:9], v4, v9, 0
; GISEL-NEXT:    v_mad_u64_u32 v[6:7], s[8:9], v5, v9, v[1:2]
; GISEL-NEXT:    v_mov_b32_e32 v1, v6
; GISEL-NEXT:    v_mad_u64_u32 v[1:2], s[8:9], v4, v8, v[1:2]
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v7, v2
; GISEL-NEXT:    v_addc_co_u32_e64 v3, s[8:9], 0, 0, vcc
; GISEL-NEXT:    v_mad_u64_u32 v[2:3], s[8:9], v5, v8, v[2:3]
; GISEL-NEXT:    v_mul_lo_u32 v5, v8, v5
; GISEL-NEXT:    v_mad_u64_u32 v[2:3], s[8:9], v8, v4, v[2:3]
; GISEL-NEXT:    v_mul_lo_u32 v4, v8, v4
; GISEL-NEXT:    v_add3_u32 v3, v4, v3, v5
; GISEL-NEXT:  .LBB0_5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:  .LBB0_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptosi double %x to i128
  ret i128 %cvt
}

define i128 @fptoui_f64_to_i128(double %x) {
; SDAG-LABEL: fptoui_f64_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_bfe_u32 v6, v1, 20, 11
; SDAG-NEXT:    v_mov_b32_e32 v7, 0
; SDAG-NEXT:    s_mov_b64 s[4:5], 0x3ff
; SDAG-NEXT:    v_cmp_gt_u64_e64 s[6:7], s[4:5], v[6:7]
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    v_mov_b32_e32 v4, 0
; SDAG-NEXT:    v_mov_b32_e32 v5, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB1_5
; SDAG-NEXT:  .LBB1_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    s_mov_b64 s[4:5], 0x432
; SDAG-NEXT:    v_cmp_lt_u64_e32 vcc, s[4:5], v[6:7]
; SDAG-NEXT:    v_and_b32_e32 v1, 0xfffff, v1
; SDAG-NEXT:    v_or_b32_e32 v1, 0x100000, v1
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB1_3
; SDAG-NEXT:  .LBB1_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u32_e32 v4, 0x433, v6
; SDAG-NEXT:    v_lshrrev_b64 v[4:5], v4, v[0:1]
; SDAG-NEXT:  .LBB1_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[4:5], exec, vcc
; SDAG-NEXT:    s_and_b64 s[4:5], s[4:5], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[4:5]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB1_5
; SDAG-NEXT:  .LBB1_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_sub_u32_e32 v2, 0x473, v6
; SDAG-NEXT:    v_add_u32_e32 v4, 0xfffffb8d, v6
; SDAG-NEXT:    v_add_u32_e32 v7, 0xfffffbcd, v6
; SDAG-NEXT:    v_lshrrev_b64 v[2:3], v2, v[0:1]
; SDAG-NEXT:    v_lshlrev_b64 v[4:5], v4, v[0:1]
; SDAG-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v7
; SDAG-NEXT:    v_lshlrev_b64 v[0:1], v7, v[0:1]
; SDAG-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; SDAG-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v7
; SDAG-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; SDAG-NEXT:    v_cndmask_b32_e64 v3, 0, v3, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e64 v2, 0, v2, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e32 v5, 0, v1, vcc
; SDAG-NEXT:    v_cndmask_b32_e32 v4, 0, v0, vcc
; SDAG-NEXT:  .LBB1_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    v_mov_b32_e32 v0, v4
; SDAG-NEXT:    v_mov_b32_e32 v1, v5
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptoui_f64_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    v_mov_b32_e32 v5, v1
; GISEL-NEXT:    v_lshrrev_b64 v[0:1], 52, v[4:5]
; GISEL-NEXT:    v_mov_b32_e32 v7, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0x3ff
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_bfe_u32 v6, v0, 0, 11
; GISEL-NEXT:    s_mov_b64 s[4:5], 0
; GISEL-NEXT:    v_cmp_ge_u64_e32 vcc, v[6:7], v[1:2]
; GISEL-NEXT:    s_mov_b64 s[6:7], s[4:5]
; GISEL-NEXT:    v_mov_b32_e32 v0, s4
; GISEL-NEXT:    v_mov_b32_e32 v1, s5
; GISEL-NEXT:    v_mov_b32_e32 v2, s6
; GISEL-NEXT:    v_mov_b32_e32 v3, s7
; GISEL-NEXT:    s_and_saveexec_b64 s[4:5], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB1_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    v_mov_b32_e32 v0, 0x433
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0xfffff
; GISEL-NEXT:    v_mov_b32_e32 v3, 0x100000
; GISEL-NEXT:    v_cmp_ge_u64_e32 vcc, v[6:7], v[0:1]
; GISEL-NEXT:    v_and_or_b32 v5, v5, v2, v3
; GISEL-NEXT:    ; implicit-def: $vgpr0_vgpr1_vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_xor_b64 s[6:7], exec, s[6:7]
; GISEL-NEXT:    s_cbranch_execz .LBB1_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_add_u32_e32 v7, 0xfffffbcd, v6
; GISEL-NEXT:    v_add_u32_e32 v6, 0xfffffb8d, v6
; GISEL-NEXT:    v_sub_u32_e32 v2, 64, v7
; GISEL-NEXT:    v_lshlrev_b64 v[0:1], v7, v[4:5]
; GISEL-NEXT:    v_lshrrev_b64 v[2:3], v2, v[4:5]
; GISEL-NEXT:    v_lshlrev_b64 v[4:5], v6, v[4:5]
; GISEL-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v7
; GISEL-NEXT:    v_cndmask_b32_e32 v0, 0, v0, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v1, 0, v1, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; GISEL-NEXT:    v_cmp_eq_u32_e32 vcc, 0, v7
; GISEL-NEXT:    v_cndmask_b32_e64 v2, v2, 0, vcc
; GISEL-NEXT:    v_cndmask_b32_e64 v3, v3, 0, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr6
; GISEL-NEXT:    ; implicit-def: $vgpr4_vgpr5
; GISEL-NEXT:  .LBB1_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[6:7], s[6:7]
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_co_u32_e32 v0, vcc, 0x433, v6
; GISEL-NEXT:    v_lshrrev_b64 v[0:1], v0, v[4:5]
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:  ; %bb.5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:  .LBB1_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptoui double %x to i128
  ret i128 %cvt
}

define i128 @fptosi_f32_to_i128(float %x) {
; SDAG-LABEL: fptosi_f32_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_mov_b32_e32 v4, v0
; SDAG-NEXT:    v_bfe_u32 v7, v4, 23, 8
; SDAG-NEXT:    s_movk_i32 s4, 0x7f
; SDAG-NEXT:    v_cmp_gt_u32_e64 s[6:7], s4, v7
; SDAG-NEXT:    v_mov_b32_e32 v0, 0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB2_5
; SDAG-NEXT:  .LBB2_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    v_ashrrev_i32_e32 v3, 31, v4
; SDAG-NEXT:    v_and_b32_e32 v0, 0x7fffff, v4
; SDAG-NEXT:    s_movk_i32 s4, 0x95
; SDAG-NEXT:    v_ashrrev_i32_e32 v5, 31, v3
; SDAG-NEXT:    v_or_b32_e32 v6, 1, v3
; SDAG-NEXT:    v_cmp_lt_u32_e32 vcc, s4, v7
; SDAG-NEXT:    v_or_b32_e32 v1, 0x800000, v0
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB2_3
; SDAG-NEXT:  .LBB2_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u32_e32 v0, 0x96, v7
; SDAG-NEXT:    v_lshrrev_b32_e32 v7, v0, v1
; SDAG-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v7, v6, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v6, v2
; SDAG-NEXT:    v_mad_u64_u32 v[4:5], s[4:5], v7, v5, v[1:2]
; SDAG-NEXT:    v_mad_i64_i32 v[2:3], s[4:5], v3, v7, v[5:6]
; SDAG-NEXT:    v_mov_b32_e32 v1, v4
; SDAG-NEXT:  .LBB2_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[4:5], exec, vcc
; SDAG-NEXT:    s_and_b64 s[4:5], s[4:5], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[4:5]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB2_5
; SDAG-NEXT:  .LBB2_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_sub_u32_e32 v4, 0xd6, v7
; SDAG-NEXT:    v_lshrrev_b64 v[8:9], v4, v[1:2]
; SDAG-NEXT:    v_add_u32_e32 v4, 0xffffff2a, v7
; SDAG-NEXT:    v_add_u32_e32 v0, 0xffffff6a, v7
; SDAG-NEXT:    v_lshlrev_b64 v[10:11], v4, v[1:2]
; SDAG-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v0
; SDAG-NEXT:    v_cndmask_b32_e32 v4, v11, v9, vcc
; SDAG-NEXT:    v_cndmask_b32_e32 v9, v10, v8, vcc
; SDAG-NEXT:    v_lshlrev_b64 v[7:8], v0, v[1:2]
; SDAG-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v0
; SDAG-NEXT:    v_cndmask_b32_e32 v11, 0, v7, vcc
; SDAG-NEXT:    v_cndmask_b32_e64 v4, 0, v4, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e64 v9, 0, v9, s[4:5]
; SDAG-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v11, v6, 0
; SDAG-NEXT:    v_cndmask_b32_e32 v13, 0, v8, vcc
; SDAG-NEXT:    v_mul_lo_u32 v12, v5, v9
; SDAG-NEXT:    v_mul_lo_u32 v4, v6, v4
; SDAG-NEXT:    v_mad_u64_u32 v[9:10], s[4:5], v6, v9, 0
; SDAG-NEXT:    v_mad_u64_u32 v[6:7], s[4:5], v13, v6, v[1:2]
; SDAG-NEXT:    v_add3_u32 v10, v10, v4, v12
; SDAG-NEXT:    v_mov_b32_e32 v1, v6
; SDAG-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v11, v5, v[1:2]
; SDAG-NEXT:    v_mad_u64_u32 v[8:9], s[4:5], v3, v11, v[9:10]
; SDAG-NEXT:    v_mul_lo_u32 v4, v3, v13
; SDAG-NEXT:    v_mul_lo_u32 v6, v3, v11
; SDAG-NEXT:    v_add_co_u32_e32 v2, vcc, v7, v2
; SDAG-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, vcc
; SDAG-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v13, v5, v[2:3]
; SDAG-NEXT:    v_add3_u32 v4, v6, v9, v4
; SDAG-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v8
; SDAG-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; SDAG-NEXT:  .LBB2_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptosi_f32_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    v_bfe_u32 v9, v4, 23, 8
; GISEL-NEXT:    s_movk_i32 s4, 0x7e
; GISEL-NEXT:    v_mov_b32_e32 v0, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    v_cmp_lt_u32_e32 vcc, s4, v9
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB2_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    v_ashrrev_i32_e32 v5, 31, v4
; GISEL-NEXT:    v_and_b32_e32 v0, 0x7fffff, v4
; GISEL-NEXT:    s_movk_i32 s4, 0x95
; GISEL-NEXT:    v_ashrrev_i32_e32 v7, 31, v5
; GISEL-NEXT:    v_or_b32_e32 v8, 1, v5
; GISEL-NEXT:    v_or_b32_e32 v1, 0x800000, v0
; GISEL-NEXT:    v_cmp_lt_u32_e32 vcc, s4, v9
; GISEL-NEXT:    ; implicit-def: $vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[4:5], vcc
; GISEL-NEXT:    s_xor_b64 s[8:9], exec, s[4:5]
; GISEL-NEXT:    s_cbranch_execz .LBB2_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_sub_u32_e32 v3, 0xd6, v9
; GISEL-NEXT:    v_add_u32_e32 v6, 0xffffff2a, v9
; GISEL-NEXT:    v_add_u32_e32 v0, 0xffffff6a, v9
; GISEL-NEXT:    v_lshrrev_b64 v[3:4], v3, v[1:2]
; GISEL-NEXT:    v_lshlrev_b64 v[9:10], v6, v[1:2]
; GISEL-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v0
; GISEL-NEXT:    v_cndmask_b32_e32 v4, v10, v4, vcc
; GISEL-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v0
; GISEL-NEXT:    v_cndmask_b32_e64 v6, 0, v4, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v9, v9, v3, vcc
; GISEL-NEXT:    v_lshlrev_b64 v[3:4], v0, v[1:2]
; GISEL-NEXT:    v_cndmask_b32_e64 v9, 0, v9, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v11, 0, v3, vcc
; GISEL-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v11, v8, 0
; GISEL-NEXT:    v_cndmask_b32_e32 v13, 0, v4, vcc
; GISEL-NEXT:    v_mul_lo_u32 v12, v7, v9
; GISEL-NEXT:    v_mul_lo_u32 v6, v8, v6
; GISEL-NEXT:    v_mad_u64_u32 v[3:4], s[4:5], v13, v8, v[1:2]
; GISEL-NEXT:    v_mad_u64_u32 v[9:10], s[4:5], v8, v9, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, v3
; GISEL-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v11, v7, v[1:2]
; GISEL-NEXT:    v_add3_u32 v10, v10, v6, v12
; GISEL-NEXT:    v_mad_u64_u32 v[8:9], s[4:5], v5, v11, v[9:10]
; GISEL-NEXT:    v_mul_lo_u32 v6, v5, v13
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v4, v2
; GISEL-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, vcc
; GISEL-NEXT:    v_mul_lo_u32 v5, v5, v11
; GISEL-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v13, v7, v[2:3]
; GISEL-NEXT:    ; implicit-def: $vgpr7
; GISEL-NEXT:    v_add3_u32 v4, v5, v9, v6
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v8
; GISEL-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr9
; GISEL-NEXT:    ; implicit-def: $vgpr8
; GISEL-NEXT:    ; implicit-def: $vgpr5_vgpr6
; GISEL-NEXT:  .LBB2_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[4:5], s[8:9]
; GISEL-NEXT:    s_cbranch_execz .LBB2_5
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_u32_e32 v0, 0x96, v9
; GISEL-NEXT:    v_lshrrev_b32_e32 v3, v0, v1
; GISEL-NEXT:    v_mad_u64_u32 v[0:1], s[8:9], v3, v8, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v8, v2
; GISEL-NEXT:    v_mad_u64_u32 v[6:7], s[8:9], v3, v7, v[1:2]
; GISEL-NEXT:    v_mad_i64_i32 v[2:3], s[8:9], v5, v3, v[7:8]
; GISEL-NEXT:    v_mov_b32_e32 v1, v6
; GISEL-NEXT:  .LBB2_5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:  .LBB2_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptosi float %x to i128
  ret i128 %cvt
}

define i128 @fptoui_f32_to_i128(float %x) {
; SDAG-LABEL: fptoui_f32_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_mov_b32_e32 v4, v0
; SDAG-NEXT:    v_bfe_u32 v5, v4, 23, 8
; SDAG-NEXT:    s_movk_i32 s4, 0x7f
; SDAG-NEXT:    v_cmp_gt_u32_e64 s[6:7], s4, v5
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    v_mov_b32_e32 v0, 0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB3_5
; SDAG-NEXT:  .LBB3_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    v_and_b32_e32 v0, 0x7fffff, v4
; SDAG-NEXT:    s_movk_i32 s4, 0x95
; SDAG-NEXT:    v_cmp_lt_u32_e32 vcc, s4, v5
; SDAG-NEXT:    v_or_b32_e32 v0, 0x800000, v0
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB3_3
; SDAG-NEXT:  .LBB3_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u32_e32 v1, 0x96, v5
; SDAG-NEXT:    v_lshrrev_b32_e32 v0, v1, v0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:  .LBB3_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[4:5], exec, vcc
; SDAG-NEXT:    s_and_b64 s[4:5], s[4:5], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[4:5]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB3_5
; SDAG-NEXT:  .LBB3_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    v_sub_u32_e32 v2, 0xd6, v5
; SDAG-NEXT:    v_add_u32_e32 v4, 0xffffff2a, v5
; SDAG-NEXT:    v_add_u32_e32 v6, 0xffffff6a, v5
; SDAG-NEXT:    v_lshrrev_b64 v[2:3], v2, v[0:1]
; SDAG-NEXT:    v_lshlrev_b64 v[4:5], v4, v[0:1]
; SDAG-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v6
; SDAG-NEXT:    v_lshlrev_b64 v[0:1], v6, v[0:1]
; SDAG-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; SDAG-NEXT:    v_cmp_ne_u32_e64 s[4:5], 0, v6
; SDAG-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; SDAG-NEXT:    v_cndmask_b32_e64 v3, 0, v3, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e64 v2, 0, v2, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e32 v1, 0, v1, vcc
; SDAG-NEXT:    v_cndmask_b32_e32 v0, 0, v0, vcc
; SDAG-NEXT:  .LBB3_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptoui_f32_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    s_mov_b64 s[4:5], 0
; GISEL-NEXT:    v_bfe_u32 v6, v4, 23, 8
; GISEL-NEXT:    v_mov_b32_e32 v0, 0x7f
; GISEL-NEXT:    s_mov_b64 s[6:7], s[4:5]
; GISEL-NEXT:    v_cmp_ge_u32_e32 vcc, v6, v0
; GISEL-NEXT:    v_mov_b32_e32 v0, s4
; GISEL-NEXT:    v_mov_b32_e32 v1, s5
; GISEL-NEXT:    v_mov_b32_e32 v2, s6
; GISEL-NEXT:    v_mov_b32_e32 v3, s7
; GISEL-NEXT:    s_and_saveexec_b64 s[4:5], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB3_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    v_mov_b32_e32 v0, 0x7fffff
; GISEL-NEXT:    v_mov_b32_e32 v1, 0x800000
; GISEL-NEXT:    v_and_or_b32 v4, v4, v0, v1
; GISEL-NEXT:    v_mov_b32_e32 v0, 0x96
; GISEL-NEXT:    v_cmp_ge_u32_e32 vcc, v6, v0
; GISEL-NEXT:    ; implicit-def: $vgpr0_vgpr1_vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_xor_b64 s[6:7], exec, s[6:7]
; GISEL-NEXT:    s_cbranch_execz .LBB3_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_add_u32_e32 v7, 0xffffff6a, v6
; GISEL-NEXT:    v_mov_b32_e32 v5, 0
; GISEL-NEXT:    v_add_u32_e32 v6, 0xffffff2a, v6
; GISEL-NEXT:    v_sub_u32_e32 v2, 64, v7
; GISEL-NEXT:    v_lshlrev_b64 v[0:1], v7, v[4:5]
; GISEL-NEXT:    v_lshrrev_b64 v[2:3], v2, v[4:5]
; GISEL-NEXT:    v_lshlrev_b64 v[4:5], v6, v[4:5]
; GISEL-NEXT:    v_cmp_gt_u32_e32 vcc, 64, v7
; GISEL-NEXT:    v_cndmask_b32_e32 v0, 0, v0, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v1, 0, v1, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; GISEL-NEXT:    v_cmp_eq_u32_e32 vcc, 0, v7
; GISEL-NEXT:    v_cndmask_b32_e64 v2, v2, 0, vcc
; GISEL-NEXT:    v_cndmask_b32_e64 v3, v3, 0, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr6
; GISEL-NEXT:    ; implicit-def: $vgpr4
; GISEL-NEXT:  .LBB3_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[6:7], s[6:7]
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_u32_e32 v0, 0x96, v6
; GISEL-NEXT:    v_lshrrev_b32_e32 v0, v0, v4
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:  ; %bb.5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:  .LBB3_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptoui float %x to i128
  ret i128 %cvt
}

define i128 @fptosi_f16_to_i128(half %x) {
; GCN-LABEL: fptosi_f16_to_i128:
; GCN:       ; %bb.0:
; GCN-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GCN-NEXT:    v_cvt_f32_f16_e32 v0, v0
; GCN-NEXT:    v_cvt_i32_f32_e32 v0, v0
; GCN-NEXT:    v_ashrrev_i32_e32 v1, 31, v0
; GCN-NEXT:    v_mov_b32_e32 v2, v1
; GCN-NEXT:    v_mov_b32_e32 v3, v1
; GCN-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptosi half %x to i128
  ret i128 %cvt
}

define i128 @fptoui_f16_to_i128(half %x) {
; GCN-LABEL: fptoui_f16_to_i128:
; GCN:       ; %bb.0:
; GCN-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GCN-NEXT:    v_cvt_f32_f16_e32 v0, v0
; GCN-NEXT:    v_mov_b32_e32 v1, 0
; GCN-NEXT:    v_mov_b32_e32 v2, 0
; GCN-NEXT:    v_mov_b32_e32 v3, 0
; GCN-NEXT:    v_cvt_u32_f32_e32 v0, v0
; GCN-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptoui half %x to i128
  ret i128 %cvt
}

define i128 @fptosi_bf16_to_i128(bfloat %x) {
; SDAG-LABEL: fptosi_bf16_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_mov_b32_e32 v4, v0
; SDAG-NEXT:    v_bfe_u32 v7, v4, 7, 8
; SDAG-NEXT:    s_movk_i32 s4, 0x7f
; SDAG-NEXT:    v_cmp_gt_u16_e64 s[6:7], s4, v7
; SDAG-NEXT:    v_mov_b32_e32 v0, 0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB6_5
; SDAG-NEXT:  .LBB6_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    v_ashrrev_i16_e32 v0, 15, v4
; SDAG-NEXT:    v_bfe_i32 v5, v0, 0, 16
; SDAG-NEXT:    v_and_b32_e32 v0, 0x7f, v4
; SDAG-NEXT:    s_movk_i32 s5, 0x85
; SDAG-NEXT:    s_mov_b32 s4, 0
; SDAG-NEXT:    v_ashrrev_i32_e32 v3, 31, v5
; SDAG-NEXT:    v_or_b32_e32 v6, 1, v5
; SDAG-NEXT:    v_cmp_lt_u16_e32 vcc, s5, v7
; SDAG-NEXT:    v_or_b32_e32 v0, 0x80, v0
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB6_3
; SDAG-NEXT:  .LBB6_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u16_e32 v1, 0x86, v7
; SDAG-NEXT:    v_lshrrev_b16_e32 v0, v1, v0
; SDAG-NEXT:    v_mul_hi_i32_i24_e32 v1, v0, v6
; SDAG-NEXT:    v_ashrrev_i32_e32 v2, 31, v1
; SDAG-NEXT:    v_mul_i32_i24_e32 v0, v0, v6
; SDAG-NEXT:    v_mov_b32_e32 v3, v2
; SDAG-NEXT:  .LBB6_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[8:9], exec, vcc
; SDAG-NEXT:    s_and_b64 s[8:9], s[8:9], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[8:9]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB6_5
; SDAG-NEXT:  .LBB6_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_add_u16_e32 v4, 0xff7a, v7
; SDAG-NEXT:    v_and_b32_e32 v0, 0xffff, v0
; SDAG-NEXT:    v_mov_b32_e32 v1, s4
; SDAG-NEXT:    v_sub_u32_e32 v2, 64, v4
; SDAG-NEXT:    v_lshrrev_b64 v[7:8], v2, v[0:1]
; SDAG-NEXT:    v_subrev_u32_e32 v2, 64, v4
; SDAG-NEXT:    v_lshlrev_b64 v[9:10], v2, v[0:1]
; SDAG-NEXT:    v_cmp_gt_u16_e32 vcc, 64, v4
; SDAG-NEXT:    v_cndmask_b32_e32 v8, v10, v8, vcc
; SDAG-NEXT:    v_cmp_ne_u16_e64 s[4:5], 0, v4
; SDAG-NEXT:    v_cndmask_b32_e64 v10, 0, v8, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e32 v9, v9, v7, vcc
; SDAG-NEXT:    v_lshlrev_b64 v[7:8], v4, v[0:1]
; SDAG-NEXT:    v_cndmask_b32_e64 v4, 0, v9, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e32 v11, 0, v7, vcc
; SDAG-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v11, v6, 0
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_cndmask_b32_e32 v14, 0, v8, vcc
; SDAG-NEXT:    v_mul_lo_u32 v13, v6, v10
; SDAG-NEXT:    v_mad_u64_u32 v[9:10], s[4:5], v6, v4, 0
; SDAG-NEXT:    v_mad_u64_u32 v[6:7], s[4:5], v14, v6, v[1:2]
; SDAG-NEXT:    v_mul_lo_u32 v12, v3, v4
; SDAG-NEXT:    v_mov_b32_e32 v1, v6
; SDAG-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v11, v3, v[1:2]
; SDAG-NEXT:    v_add3_u32 v10, v10, v13, v12
; SDAG-NEXT:    v_mad_u64_u32 v[8:9], s[4:5], v5, v11, v[9:10]
; SDAG-NEXT:    v_mul_lo_u32 v6, v5, v14
; SDAG-NEXT:    v_add_co_u32_e32 v4, vcc, v7, v2
; SDAG-NEXT:    v_mul_lo_u32 v10, v5, v11
; SDAG-NEXT:    v_addc_co_u32_e64 v5, s[4:5], 0, 0, vcc
; SDAG-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v14, v3, v[4:5]
; SDAG-NEXT:    v_add3_u32 v4, v10, v9, v6
; SDAG-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v8
; SDAG-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; SDAG-NEXT:  .LBB6_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptosi_bf16_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    v_bfe_u32 v8, v4, 7, 8
; GISEL-NEXT:    s_movk_i32 s4, 0x7e
; GISEL-NEXT:    v_mov_b32_e32 v0, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    v_cmp_lt_u16_e32 vcc, s4, v8
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB6_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    v_ashrrev_i16_e32 v0, 15, v4
; GISEL-NEXT:    v_bfe_i32 v7, v0, 0, 16
; GISEL-NEXT:    v_and_b32_e32 v0, 0x7f, v4
; GISEL-NEXT:    s_movk_i32 s5, 0x85
; GISEL-NEXT:    s_mov_b32 s4, 0
; GISEL-NEXT:    v_ashrrev_i32_e32 v6, 31, v7
; GISEL-NEXT:    v_or_b32_e32 v5, 1, v7
; GISEL-NEXT:    v_or_b32_e32 v4, 0x80, v0
; GISEL-NEXT:    v_cmp_lt_u16_e32 vcc, s5, v8
; GISEL-NEXT:    ; implicit-def: $vgpr0_vgpr1
; GISEL-NEXT:    ; implicit-def: $vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[8:9], vcc
; GISEL-NEXT:    s_xor_b64 s[8:9], exec, s[8:9]
; GISEL-NEXT:    s_cbranch_execz .LBB6_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_add_u16_e32 v10, 0xff7a, v8
; GISEL-NEXT:    v_and_b32_e32 v0, 0xffff, v4
; GISEL-NEXT:    v_mov_b32_e32 v1, s4
; GISEL-NEXT:    v_sub_u32_e32 v2, 64, v10
; GISEL-NEXT:    v_lshrrev_b64 v[3:4], v2, v[0:1]
; GISEL-NEXT:    v_subrev_u32_e32 v2, 64, v10
; GISEL-NEXT:    v_lshlrev_b64 v[8:9], v2, v[0:1]
; GISEL-NEXT:    v_cmp_gt_u16_e32 vcc, 64, v10
; GISEL-NEXT:    v_cndmask_b32_e32 v4, v9, v4, vcc
; GISEL-NEXT:    v_cmp_ne_u16_e64 s[4:5], 0, v10
; GISEL-NEXT:    v_cndmask_b32_e64 v9, 0, v4, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v8, v8, v3, vcc
; GISEL-NEXT:    v_lshlrev_b64 v[3:4], v10, v[0:1]
; GISEL-NEXT:    v_cndmask_b32_e64 v8, 0, v8, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v10, 0, v3, vcc
; GISEL-NEXT:    v_mad_u64_u32 v[0:1], s[4:5], v10, v5, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_cndmask_b32_e32 v13, 0, v4, vcc
; GISEL-NEXT:    v_mul_lo_u32 v11, v6, v8
; GISEL-NEXT:    v_mad_u64_u32 v[3:4], s[4:5], v13, v5, v[1:2]
; GISEL-NEXT:    v_mul_lo_u32 v12, v5, v9
; GISEL-NEXT:    v_mad_u64_u32 v[8:9], s[4:5], v5, v8, 0
; GISEL-NEXT:    v_mul_lo_u32 v5, v7, v13
; GISEL-NEXT:    v_mov_b32_e32 v1, v3
; GISEL-NEXT:    v_mad_u64_u32 v[1:2], s[4:5], v10, v6, v[1:2]
; GISEL-NEXT:    v_add3_u32 v9, v9, v12, v11
; GISEL-NEXT:    v_mad_u64_u32 v[8:9], s[4:5], v7, v10, v[8:9]
; GISEL-NEXT:    v_mul_lo_u32 v7, v7, v10
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v4, v2
; GISEL-NEXT:    v_addc_co_u32_e64 v3, s[4:5], 0, 0, vcc
; GISEL-NEXT:    v_mad_u64_u32 v[2:3], s[4:5], v13, v6, v[2:3]
; GISEL-NEXT:    v_add3_u32 v4, v7, v9, v5
; GISEL-NEXT:    ; implicit-def: $vgpr5
; GISEL-NEXT:    v_add_co_u32_e32 v2, vcc, v2, v8
; GISEL-NEXT:    v_addc_co_u32_e32 v3, vcc, v3, v4, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr8
; GISEL-NEXT:    ; implicit-def: $vgpr4
; GISEL-NEXT:  .LBB6_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[4:5], s[8:9]
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_u16_e32 v0, 0x86, v8
; GISEL-NEXT:    v_lshrrev_b16_e32 v0, v0, v4
; GISEL-NEXT:    v_mul_hi_i32_i24_e32 v1, v0, v5
; GISEL-NEXT:    v_ashrrev_i32_e32 v2, 31, v1
; GISEL-NEXT:    v_mul_i32_i24_e32 v0, v0, v5
; GISEL-NEXT:    v_mov_b32_e32 v3, v2
; GISEL-NEXT:  ; %bb.5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:  .LBB6_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptosi bfloat %x to i128
  ret i128 %cvt
}

define i128 @fptoui_bf16_to_i128(bfloat %x) {
; SDAG-LABEL: fptoui_bf16_to_i128:
; SDAG:       ; %bb.0: ; %fp-to-i-entry
; SDAG-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; SDAG-NEXT:    v_mov_b32_e32 v4, v0
; SDAG-NEXT:    v_bfe_u32 v5, v4, 7, 8
; SDAG-NEXT:    s_movk_i32 s4, 0x7f
; SDAG-NEXT:    v_cmp_gt_u16_e64 s[6:7], s4, v5
; SDAG-NEXT:    v_mov_b32_e32 v2, 0
; SDAG-NEXT:    v_mov_b32_e32 v3, 0
; SDAG-NEXT:    v_mov_b32_e32 v0, 0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:    s_xor_b64 exec, s[6:7], exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB7_5
; SDAG-NEXT:  .LBB7_1: ; %fp-to-i-if-check.exp.size
; SDAG-NEXT:    v_and_b32_e32 v0, 0x7f, v4
; SDAG-NEXT:    s_movk_i32 s4, 0x85
; SDAG-NEXT:    v_cmp_lt_u16_e32 vcc, s4, v5
; SDAG-NEXT:    v_or_b32_e32 v0, 0x80, v0
; SDAG-NEXT:    s_xor_b64 exec, vcc, exec
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB7_3
; SDAG-NEXT:  .LBB7_2: ; %fp-to-i-if-exp.small
; SDAG-NEXT:    v_sub_u16_e32 v1, 0x86, v5
; SDAG-NEXT:    v_lshrrev_b16_e32 v0, v1, v0
; SDAG-NEXT:    v_mov_b32_e32 v1, 0
; SDAG-NEXT:  .LBB7_3:
; SDAG-NEXT:    s_or_b64 exec, exec, vcc
; SDAG-NEXT:    s_xor_b64 s[4:5], exec, vcc
; SDAG-NEXT:    s_and_b64 s[4:5], s[4:5], exec
; SDAG-NEXT:    s_or_b64 s[6:7], s[6:7], s[4:5]
; SDAG-NEXT:    s_mov_b64 exec, vcc
; SDAG-NEXT:    ; divergent control-flow edge
; SDAG-NEXT:    s_cbranch_execz .LBB7_5
; SDAG-NEXT:  .LBB7_4: ; %fp-to-i-if-exp.large
; SDAG-NEXT:    v_add_u16_e32 v6, 0xff7a, v5
; SDAG-NEXT:    s_mov_b32 s4, 0
; SDAG-NEXT:    v_and_b32_e32 v0, 0xffff, v0
; SDAG-NEXT:    v_mov_b32_e32 v1, s4
; SDAG-NEXT:    v_sub_u32_e32 v2, 64, v6
; SDAG-NEXT:    v_subrev_u32_e32 v4, 64, v6
; SDAG-NEXT:    v_lshrrev_b64 v[2:3], v2, v[0:1]
; SDAG-NEXT:    v_lshlrev_b64 v[4:5], v4, v[0:1]
; SDAG-NEXT:    v_cmp_gt_u16_e32 vcc, 64, v6
; SDAG-NEXT:    v_lshlrev_b64 v[0:1], v6, v[0:1]
; SDAG-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; SDAG-NEXT:    v_cmp_ne_u16_e64 s[4:5], 0, v6
; SDAG-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; SDAG-NEXT:    v_cndmask_b32_e64 v3, 0, v3, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e64 v2, 0, v2, s[4:5]
; SDAG-NEXT:    v_cndmask_b32_e32 v1, 0, v1, vcc
; SDAG-NEXT:    v_cndmask_b32_e32 v0, 0, v0, vcc
; SDAG-NEXT:  .LBB7_5: ; %fp-to-i-cleanup
; SDAG-NEXT:    s_or_b64 exec, exec, s[6:7]
; SDAG-NEXT:    s_setpc_b64 s[30:31]
;
; GISEL-LABEL: fptoui_bf16_to_i128:
; GISEL:       ; %bb.0: ; %fp-to-i-entry
; GISEL-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GISEL-NEXT:    v_mov_b32_e32 v4, v0
; GISEL-NEXT:    v_bfe_u32 v5, v4, 7, 8
; GISEL-NEXT:    s_movk_i32 s4, 0x7e
; GISEL-NEXT:    v_mov_b32_e32 v0, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    v_cmp_lt_u16_e32 vcc, s4, v5
; GISEL-NEXT:    s_and_saveexec_b64 s[6:7], vcc
; GISEL-NEXT:    s_cbranch_execz .LBB7_6
; GISEL-NEXT:  ; %bb.1: ; %fp-to-i-if-check.exp.size
; GISEL-NEXT:    v_and_b32_e32 v0, 0x7f, v4
; GISEL-NEXT:    s_movk_i32 s4, 0x85
; GISEL-NEXT:    v_or_b32_e32 v4, 0x80, v0
; GISEL-NEXT:    v_cmp_lt_u16_e32 vcc, s4, v5
; GISEL-NEXT:    ; implicit-def: $vgpr0_vgpr1
; GISEL-NEXT:    ; implicit-def: $vgpr2_vgpr3
; GISEL-NEXT:    s_and_saveexec_b64 s[4:5], vcc
; GISEL-NEXT:    s_xor_b64 s[8:9], exec, s[4:5]
; GISEL-NEXT:    s_cbranch_execz .LBB7_3
; GISEL-NEXT:  ; %bb.2: ; %fp-to-i-if-exp.large
; GISEL-NEXT:    v_add_u16_e32 v6, 0xff7a, v5
; GISEL-NEXT:    s_mov_b32 s4, 0
; GISEL-NEXT:    v_and_b32_e32 v0, 0xffff, v4
; GISEL-NEXT:    v_mov_b32_e32 v1, s4
; GISEL-NEXT:    v_sub_u32_e32 v2, 64, v6
; GISEL-NEXT:    v_subrev_u32_e32 v4, 64, v6
; GISEL-NEXT:    v_lshrrev_b64 v[2:3], v2, v[0:1]
; GISEL-NEXT:    v_lshlrev_b64 v[4:5], v4, v[0:1]
; GISEL-NEXT:    v_cmp_gt_u16_e32 vcc, 64, v6
; GISEL-NEXT:    v_lshlrev_b64 v[0:1], v6, v[0:1]
; GISEL-NEXT:    v_cndmask_b32_e32 v3, v5, v3, vcc
; GISEL-NEXT:    v_cmp_ne_u16_e64 s[4:5], 0, v6
; GISEL-NEXT:    v_cndmask_b32_e32 v2, v4, v2, vcc
; GISEL-NEXT:    v_cndmask_b32_e64 v3, 0, v3, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e64 v2, 0, v2, s[4:5]
; GISEL-NEXT:    v_cndmask_b32_e32 v1, 0, v1, vcc
; GISEL-NEXT:    v_cndmask_b32_e32 v0, 0, v0, vcc
; GISEL-NEXT:    ; implicit-def: $vgpr5
; GISEL-NEXT:    ; implicit-def: $vgpr4
; GISEL-NEXT:  .LBB7_3: ; %Flow
; GISEL-NEXT:    s_andn2_saveexec_b64 s[4:5], s[8:9]
; GISEL-NEXT:  ; %bb.4: ; %fp-to-i-if-exp.small
; GISEL-NEXT:    v_sub_u16_e32 v0, 0x86, v5
; GISEL-NEXT:    v_lshrrev_b16_e32 v0, v0, v4
; GISEL-NEXT:    v_mov_b32_e32 v2, 0
; GISEL-NEXT:    v_mov_b32_e32 v3, 0
; GISEL-NEXT:    v_mov_b32_e32 v1, 0
; GISEL-NEXT:  ; %bb.5: ; %Flow1
; GISEL-NEXT:    s_or_b64 exec, exec, s[4:5]
; GISEL-NEXT:  .LBB7_6: ; %fp-to-i-cleanup
; GISEL-NEXT:    s_or_b64 exec, exec, s[6:7]
; GISEL-NEXT:    s_setpc_b64 s[30:31]
  %cvt = fptoui bfloat %x to i128
  ret i128 %cvt
}
