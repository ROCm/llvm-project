...
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel=1 -mtriple=amdgpu11.00--amdpal < %s | FileCheck -check-prefix=GISEL-GFX11 %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel=1 -mtriple=amdgpu10.30--amdpal < %s | FileCheck -check-prefix=GISEL-GFX10 %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel=0 -mtriple=amdgpu11.00--amdpal < %s | FileCheck -check-prefix=DAGISEL-GFX11 %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel=0 -mtriple=amdgpu10.30--amdpal < %s | FileCheck -check-prefix=DAGISEL-GFX10 %s
