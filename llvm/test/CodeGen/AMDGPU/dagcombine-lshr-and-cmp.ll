...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgcn-- -mcpu=gfx600 -stop-after=amdgpu-isel -O0 < %s | FileCheck -check-prefix=GCN %s
...
define i32 @divergent_lshr_and_cmp(i32 %x) {
  ...
  ; GCN-NEXT:   SI_BRCOND %bb.1, killed [[V_CMP_NE_U32_e64_]], implicit-def dead $exec, implicit-def dead $vcc, implicit $exec
...
define amdgpu_kernel void @uniform_opt_lshr_and_cmp(ptr addrspace(1) %out, i32 %x) {
  ...
  ; GCN-NEXT:   [[S_AND_B64_:%[0-9]+]]:sreg_64 = S_AND_B64 $exec, [[COPY2]], implicit-def dead $scc
