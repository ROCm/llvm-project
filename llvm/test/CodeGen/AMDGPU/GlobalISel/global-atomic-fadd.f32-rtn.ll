...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu9.0a -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefix=GFX90A %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu9.42 -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefix=GFX942 %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu11.00 -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefix=GFX11 %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu12.00 -stop-after=instruction-select -amdgpu-atomic-optimizer-strategy=DPP < %s | FileCheck -check-prefix=GFX11 %s
