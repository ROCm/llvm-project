...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -global-isel-abort=2 -mtriple=amdgcn-mesa-mesa3d -mcpu=tonga -stop-after=instruction-select -o - %s | FileCheck %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -global-isel-abort=2 -mtriple=amdgcn-mesa-mesa3d -mcpu=gfx810 -stop-after=instruction-select -o - %s | FileCheck %s
...
define amdgpu_ps void @raw_ptr_buffer_store_format__vgpr_rsrc__vgpr_val__vgpr_voffset__sgpr_soffset_v4f32(ptr addrspace(8) %rsrc, <4 x float> %val, i32 %voffset, i32 inreg %soffset) {
  ...
  ; CHECK-NEXT:   [[COPY5:%[0-9]+]]:vgpr(f32) = COPY $vgpr5
  ; CHECK-NEXT:   [[COPY6:%[0-9]+]]:vgpr(f32) = COPY $vgpr6
  ; CHECK-NEXT:   [[COPY7:%[0-9]+]]:vgpr(f32) = COPY $vgpr7
  ; CHECK-NEXT:   [[BUILD_VECTOR:%[0-9]+]]:vreg_128(<4 x f32>) = G_BUILD_VECTOR [[COPY4]](f32), [[COPY5]](f32), [[COPY6]](f32), [[COPY7]](f32)
  ; CHECK-NEXT:   [[COPY8:%[0-9]+]]:vgpr_32(i32) = COPY $vgpr8
  ; CHECK-NEXT:   [[COPY9:%[0-9]+]]:sreg_32(i32) = COPY $sgpr2
  ; CHECK-NEXT:   [[BUILD_VECTOR1:%[0-9]+]]:vreg_128(<4 x s32>) = G_BUILD_VECTOR [[COPY]](s32), [[COPY1]](s32), [[COPY2]](s32), [[COPY3]](s32)
  ...
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   BUFFER_STORE_FORMAT_XYZW_OFFEN_exact [[BUILD_VECTOR]](<4 x f32>), [[COPY8]](i32), [[REG_SEQUENCE]](<4 x s32>), [[COPY9]](i32), 0, 0, 0, implicit $exec :: (dereferenceable store (<4 x f32>) into %ir.rsrc, align 1, addrspace 8)
...
define amdgpu_ps void @raw_ptr_buffer_store_format__vgpr_rsrc__vgpr_val__vgpr_voffset__sgpr_soffset_v4f32_add_4096(ptr addrspace(8) %rsrc, <4 x float> %val, i32 %voffset, i32 inreg %soffset) {
  ...
  ; CHECK-NEXT:   [[COPY7:%[0-9]+]]:vgpr(f32) = COPY $vgpr7
  ; CHECK-NEXT:   [[BUILD_VECTOR:%[0-9]+]]:vreg_128(<4 x f32>) = G_BUILD_VECTOR [[COPY4]](f32), [[COPY5]](f32), [[COPY6]](f32), [[COPY7]](f32)
  ; CHECK-NEXT:   [[COPY8:%[0-9]+]]:vgpr(i32) = COPY $vgpr8
  ; CHECK-NEXT:   [[COPY9:%[0-9]+]]:sreg_32(i32) = COPY $sgpr2
  ; CHECK-NEXT:   [[C:%[0-9]+]]:sgpr(i32) = G_CONSTANT i32 4096
  ; CHECK-NEXT:   [[COPY10:%[0-9]+]]:vgpr(i32) = COPY [[C]](i32)
  ; CHECK-NEXT:   [[ADD:%[0-9]+]]:vgpr_32(i32) = G_ADD [[COPY8]], [[COPY10]]
  ; CHECK-NEXT:   [[BUILD_VECTOR1:%[0-9]+]]:vreg_128(<4 x s32>) = G_BUILD_VECTOR [[COPY]](s32), [[COPY1]](s32), [[COPY2]](s32), [[COPY3]](s32)
  ; CHECK-NEXT:   [[C1:%[0-9]+]]:sgpr(i32) = G_CONSTANT i32 0
  ; CHECK-NEXT:   [[COPY11:%[0-9]+]]:vgpr(i32) = COPY [[C1]](i32)
  ; CHECK-NEXT:   [[DEF:%[0-9]+]]:sreg_64_xexec = IMPLICIT_DEF
  ; CHECK-NEXT:   [[S_MOV_B64_:%[0-9]+]]:sreg_64_xexec = S_MOV_B64 $exec
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.2:
  ...
  ; CHECK-NEXT:   successors: %bb.4(0x40000000), %bb.2(0x40000000)
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   BUFFER_STORE_FORMAT_XYZW_OFFEN_exact [[BUILD_VECTOR]](<4 x f32>), [[ADD]](i32), [[REG_SEQUENCE]](<4 x s32>), [[COPY9]](i32), 0, 0, 0, implicit $exec :: (dereferenceable store (<4 x f32>) into %ir.rsrc, align 1, addrspace 8)
  ; CHECK-NEXT:   $exec = S_XOR_B64_term $exec, [[S_AND_SAVEEXEC_B64_]], implicit-def $scc
  ; CHECK-NEXT:   SI_WATERFALL_LOOP %bb.2, implicit $exec
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.4:
