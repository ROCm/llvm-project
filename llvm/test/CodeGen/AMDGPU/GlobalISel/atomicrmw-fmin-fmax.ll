...
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -mtriple=amdgpu10.10-amd-amdhsa -stop-after=instruction-select < %s | FileCheck %s --check-prefixes=GFX10
...
define void @atomicrmw_fmax_flat_f64_vv_noret(ptr %ptr, double %val) {
  ...
  ; GFX10-NEXT:   SI_BRCOND %bb.3, [[COPY8]], implicit-def $exec, implicit-def $vcc_lo, implicit $exec
  ; GFX10-NEXT:   S_BRANCH %bb.2
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT: bb.2.atomicrmw.private:
  ...
  ; GFX10-NEXT:   [[BUFFER_LOAD_DWORD_OFFEN:%[0-9]+]]:vgpr_32 = BUFFER_LOAD_DWORD_OFFEN [[V_CNDMASK_B32_e64_]], $sgpr0_sgpr1_sgpr2_sgpr3, 0, 0, 0, 0, implicit $exec :: (load (s32) from %ir.1, align 8, addrspace 5)
  ; GFX10-NEXT:   [[BUFFER_LOAD_DWORD_OFFEN1:%[0-9]+]]:vgpr_32 = BUFFER_LOAD_DWORD_OFFEN [[V_CNDMASK_B32_e64_]], $sgpr0_sgpr1_sgpr2_sgpr3, 0, 4, 0, 0, implicit $exec :: (load (s32) from %ir.1 + 4, basealign 8, addrspace 5)
  ...
  ; GFX10-NEXT:   BUFFER_STORE_DWORD_OFFEN [[COPY11]], [[V_CNDMASK_B32_e64_]], $sgpr0_sgpr1_sgpr2_sgpr3, 0, 0, 0, 0, implicit $exec :: (store (s32) into %ir.1, align 8, addrspace 5)
  ; GFX10-NEXT:   BUFFER_STORE_DWORD_OFFEN [[COPY12]], [[V_CNDMASK_B32_e64_]], $sgpr0_sgpr1_sgpr2_sgpr3, 0, 4, 0, 0, implicit $exec :: (store (s32) into %ir.1 + 4, basealign 8, addrspace 5)
  ; GFX10-NEXT:   S_BRANCH %bb.4
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT: bb.3.atomicrmw.global:
  ; GFX10-NEXT:   successors: %bb.4(0x80000000)
  ; GFX10-NEXT: {{  $}}
...
define double @atomicrmw_fmax_flat_f64_vv_ret(ptr %ptr, double %val) {
  ...
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT:   [[PHI:%[0-9]+]]:vreg_64 = PHI [[REG_SEQUENCE2]], %bb.2, [[FLAT_ATOMIC_MAX_F64_RTN]], %bb.3
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT: bb.5.atomicrmw.end:
  ; GFX10-NEXT:   [[COPY13:%[0-9]+]]:vgpr_32 = COPY [[PHI]].sub0
  ; GFX10-NEXT:   [[COPY14:%[0-9]+]]:vgpr_32 = COPY [[PHI]].sub1
...
define double @atomicrmw_fmin_flat_f64_vv_ret(ptr %ptr, double %val) {
  ...
  ; GFX10-NEXT:   successors: %bb.5(0x80000000)
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT:   [[PHI:%[0-9]+]]:vreg_64 = PHI [[REG_SEQUENCE2]], %bb.2, [[FLAT_ATOMIC_MIN_F64_RTN]], %bb.3
  ; GFX10-NEXT: {{  $}}
  ; GFX10-NEXT: bb.5.atomicrmw.end:
  ; GFX10-NEXT:   [[COPY13:%[0-9]+]]:vgpr_32 = COPY [[PHI]].sub0
  ; GFX10-NEXT:   [[COPY14:%[0-9]+]]:vgpr_32 = COPY [[PHI]].sub1
