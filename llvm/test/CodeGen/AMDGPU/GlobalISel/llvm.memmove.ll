...
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -mtriple=amdgpu6.00-- -mem-intrinsic-expand-size=3 %s -o - | FileCheck -check-prefix=LOOP %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -mtriple=amdgpu6.00-- -mem-intrinsic-expand-size=5 %s -o - | FileCheck -check-prefix=UNROLL %s
...
; LOOP-NEXT:    s_mov_b64 exec, vcc
; LOOP-NEXT:    ; divergent control-flow edge
...
; LOOP-NEXT:    buffer_load_ubyte v7, v[2:3], s[4:7], 0 addr64
; LOOP-NEXT:    s_waitcnt vmcnt(3)
; LOOP-NEXT:    v_lshlrev_b32_e32 v4, 8, v4
; LOOP-NEXT:    s_waitcnt vmcnt(2)
; LOOP-NEXT:    v_lshlrev_b32_e32 v5, 24, v5
; LOOP-NEXT:    s_waitcnt vmcnt(1)
; LOOP-NEXT:    v_lshlrev_b32_e32 v6, 16, v6
; LOOP-NEXT:    s_waitcnt vmcnt(0)
; LOOP-NEXT:    v_or_b32_e32 v4, v4, v7
; LOOP-NEXT:    v_or_b32_e32 v5, v5, v6
; LOOP-NEXT:    v_or_b32_e32 v4, v5, v4
; LOOP-NEXT:    v_bfe_u32 v6, v4, 8, 8
; LOOP-NEXT:    v_lshrrev_b32_e32 v5, 16, v4
; LOOP-NEXT:    buffer_store_byte v4, v[0:1], s[4:7], 0 addr64
; LOOP-NEXT:    s_waitcnt expcnt(0)
; LOOP-NEXT:    v_lshrrev_b32_e32 v4, 24, v4
; LOOP-NEXT:    buffer_store_byte v6, v[0:1], s[4:7], 0 addr64 offset:1
; LOOP-NEXT:    buffer_store_byte v5, v[0:1], s[4:7], 0 addr64 offset:2
; LOOP-NEXT:    buffer_store_byte v4, v[0:1], s[4:7], 0 addr64 offset:3
; LOOP-NEXT:  .LBB0_2:
; LOOP-NEXT:    s_or_b64 exec, exec, s[0:1]
; LOOP-NEXT:    s_xor_b64 s[2:3], exec, s[0:1]
; LOOP-NEXT:    s_mov_b64 exec, s[0:1]
; LOOP-NEXT:    ; divergent control-flow edge
; LOOP-NEXT:    s_cbranch_execz .LBB0_4
; LOOP-NEXT:  .LBB0_3: ; %memmove_bwd_residual
