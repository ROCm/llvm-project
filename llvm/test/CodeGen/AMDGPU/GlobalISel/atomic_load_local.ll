; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -global-isel-abort=0 -mtriple=amdgpu7.00-amd-amdhsa < %s | FileCheck -check-prefixes=GCN,CI %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -global-isel-abort=0 -mtriple=amdgpu9.00-amd-amdhsa < %s | FileCheck -check-prefixes=GCN,GFX9 %s
