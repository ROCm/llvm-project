...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu7.01-amd-amdhsa -stop-after=legalizer < %s | FileCheck -check-prefix=GCN %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu7.01-amd-amdpal -stop-after=legalizer < %s | FileCheck -check-prefix=GCN-PAL %s
