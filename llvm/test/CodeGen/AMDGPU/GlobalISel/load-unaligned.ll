...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu9.00-amd-amdpal -mattr=+unaligned-access-mode < %s | FileCheck --check-prefix=GFX9 %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu7.01-amd-amdpal -mattr=+unaligned-access-mode < %s | FileCheck --check-prefix=GFX7 %s
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel -mtriple=amdgpu10.10-amd-amdpal -mattr=+unaligned-access-mode < %s | FileCheck --check-prefix=GFX10 %s
