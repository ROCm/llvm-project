...
; RUN:  llc -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu6.00 -fp-contract=on < %s | FileCheck -check-prefixes=SI,SI-STRICT %s
; RUN:  llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu6.01  -fp-contract=on < %s | FileCheck -check-prefixes=SI,SI-STRICT %s
; RUN:  llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu6.00 -fp-contract=fast < %s | FileCheck -check-prefixes=SI,SI-CONTRACT %s
; RUN:  llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu6.01  -fp-contract=fast < %s | FileCheck -check-prefixes=SI,SI-CONTRACT %s
; RUN:  llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu8.02 -mattr=-flat-for-global  -fp-contract=on < %s | FileCheck -check-prefixes=VI,VI-STRICT %s
; RUN:  llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu8.02 -mattr=-flat-for-global  -fp-contract=fast < %s | FileCheck -check-prefixes=VI,VI-CONTRACT %s

define amdgpu_kernel void @fmuladd_f64(ptr addrspace(1) %out, ptr addrspace(1) %in1, ptr addrspace(1) %in2, ptr addrspace(1) %in3) #0 {
; SI-CONTRACT-LABEL: fmuladd_f64:
; SI-CONTRACT:       ; %bb.0:
...
; SI-CONTRACT-LABEL: fmul_fadd_contract_f64:
; SI-CONTRACT:       ; %bb.0:
; SI-CONTRACT-NEXT:    s_load_dwordx8 s[0:7], s[4:5], 0x9
; SI-CONTRACT-NEXT:    s_mov_b32 s11, 0xf000
; SI-CONTRACT-NEXT:    s_mov_b32 s10, -1
