...
; RUN: sed 's/CODE_OBJECT_VERSION/400/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa | FileCheck -check-prefixes=MUBUF,DEFAULTSIZE %s
; RUN: sed 's/CODE_OBJECT_VERSION/500/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa | FileCheck -check-prefixes=MUBUF,DEFAULTSIZE-V5 %s
; RUN: sed 's/CODE_OBJECT_VERSION/600/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa | FileCheck -check-prefixes=MUBUF,DEFAULTSIZE-V5 %s
; RUN: sed 's/CODE_OBJECT_VERSION/400/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa -amdgpu-assume-dynamic-stack-object-size=1024 | FileCheck -check-prefixes=MUBUF,ASSUME1024 %s
; RUN: sed 's/CODE_OBJECT_VERSION/400/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa -mattr=+enable-flat-scratch | FileCheck -check-prefixes=FLATSCR,DEFAULTSIZE %s
; RUN: sed 's/CODE_OBJECT_VERSION/400/g' %s | llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.00-amd-amdhsa -mattr=+enable-flat-scratch -amdgpu-assume-dynamic-stack-object-size=1024 | FileCheck -check-prefixes=FLATSCR,ASSUME1024 %s
...
; MUBUF-NEXT:    s_or_b64 exec, exec, vcc
; MUBUF-NEXT:    v_mov_b32_e32 v0, 0
; MUBUF-NEXT:    global_store_dword v[0:1], v0, off
; MUBUF-NEXT:    s_waitcnt vmcnt(0)
...
; FLATSCR-NEXT:    s_or_b64 exec, exec, vcc
; FLATSCR-NEXT:    v_mov_b32_e32 v0, 0
; FLATSCR-NEXT:    global_store_dword v[0:1], v0, off
; FLATSCR-NEXT:    s_waitcnt vmcnt(0)
; FLATSCR-NEXT:    s_mov_b32 s32, s34
; FLATSCR-NEXT:    s_mov_b32 s34, s2
; FLATSCR-NEXT:    s_mov_b32 s33, s1
; FLATSCR-NEXT:    s_setpc_b64 s[30:31]
entry:
  %cond = icmp eq i32 %arg.cond, 0
  br i1 %cond, label %bb.0, label %bb.1
