; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu8.03-amd-amdhsa -enable-ipra=0 < %s | FileCheck -enable-var-scope --check-prefix=GCN %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu7.01-amd-amdhsa -enable-ipra=0 < %s | FileCheck -enable-var-scope --check-prefix=GCN %s
