...
; RUN: llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false -mtriple=amdgcn -mcpu=tahiti -denormal-fp-math-f32=preserve-sign < %s | FileCheck -check-prefixes=GCN-DAZ,GCN-DAZ-UNSAFE,SI-DAZ-UNSAFE %s
; RUN: llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false -mtriple=amdgcn -mcpu=tahiti -denormal-fp-math-f32=ieee < %s          | FileCheck -check-prefixes=GCN-IEEE,GCN-IEEE-UNSAFE,SI-IEEE-UNSAFE %s


; RUN: llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false -mtriple=amdgcn -mcpu=hawaii -denormal-fp-math-f32=preserve-sign < %s | FileCheck -check-prefixes=GCN-DAZ,GCN-DAZ-UNSAFE,CI-DAZ-UNSAFE %s
; RUN: llc -amdgpu-late-wave-transform=1 -amdgpu-scalarize-global-loads=false -mtriple=amdgcn -mcpu=hawaii -denormal-fp-math-f32=ieee < %s          | FileCheck -check-prefixes=GCN-IEEE,GCN-IEEE-UNSAFE,CI-IEEE-UNSAFE %s
...
; GCN-IEEE-NEXT:    s_load_dword s4, s[4:5], 0xb
; GCN-IEEE-NEXT:    s_mov_b32 s3, 0xf000
; GCN-IEEE-NEXT:    s_mov_b32 s2, -1
; GCN-IEEE-NEXT:    s_waitcnt lgkmcnt(0)
; GCN-IEEE-NEXT:    v_rsq_f32_e32 v0, s4
; GCN-IEEE-NEXT:    buffer_store_dword v0, off, s[0:3], 0
; GCN-IEEE-NEXT:    s_endpgm
...
; GCN-DAZ-NEXT:    s_waitcnt lgkmcnt(0)
; GCN-DAZ-NEXT:    s_mov_b64 s[4:5], s[2:3]
; GCN-DAZ-NEXT:    buffer_load_dword v2, v[0:1], s[4:7], 0 addr64 glc
; GCN-DAZ-NEXT:    s_waitcnt vmcnt(0)
; GCN-DAZ-NEXT:    buffer_load_dword v3, v[0:1], s[4:7], 0 addr64 offset:4 glc
; GCN-DAZ-NEXT:    s_waitcnt vmcnt(0)
; GCN-DAZ-NEXT:    buffer_load_dword v4, v[0:1], s[4:7], 0 addr64 offset:8 glc
; GCN-DAZ-NEXT:    s_waitcnt vmcnt(0)
; GCN-DAZ-NEXT:    s_mov_b64 s[2:3], s[6:7]
...
; GCN-IEEE-NEXT:    s_waitcnt lgkmcnt(0)
; GCN-IEEE-NEXT:    s_mov_b64 s[4:5], s[2:3]
; GCN-IEEE-NEXT:    buffer_load_dword v2, v[0:1], s[4:7], 0 addr64 glc
; GCN-IEEE-NEXT:    s_waitcnt vmcnt(0)
; GCN-IEEE-NEXT:    buffer_load_dword v3, v[0:1], s[4:7], 0 addr64 offset:4 glc
; GCN-IEEE-NEXT:    s_waitcnt vmcnt(0)
; GCN-IEEE-NEXT:    buffer_load_dword v4, v[0:1], s[4:7], 0 addr64 offset:8 glc
; GCN-IEEE-NEXT:    s_waitcnt vmcnt(0)
; GCN-IEEE-NEXT:    s_mov_b64 s[2:3], s[6:7]
