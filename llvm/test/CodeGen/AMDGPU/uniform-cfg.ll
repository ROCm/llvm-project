...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgcn -mcpu=verde -amdgpu-early-ifcvt=0 -machine-sink-split-probability-threshold=0 -structurizecfg-skip-uniform-regions < %s | FileCheck -enable-var-scope -check-prefix=GCN -check-prefix=SI %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgcn -mcpu=tonga -mattr=-flat-for-global -amdgpu-early-ifcvt=0 -machine-sink-split-probability-threshold=0 -structurizecfg-skip-uniform-regions < %s | FileCheck -enable-var-scope -check-prefix=GCN -check-prefix=VI %s
...
; SI:       ; %bb.0: ; %entry
; SI-NEXT:    v_cmp_lt_u32_e32 vcc, 15, v0
...
; VI:       ; %bb.0: ; %entry
; VI-NEXT:    v_cmp_lt_u32_e32 vcc, 15, v0
...
; VI-NEXT:    s_and_saveexec_b64 s[6:7], s[2:3]
; VI-NEXT:    ; divergent control-flow edge
