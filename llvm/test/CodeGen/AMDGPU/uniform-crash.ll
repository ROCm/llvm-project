; RUN: llc -amdgpu-late-wave-transform=1 < %s -mtriple=amdgpu6.01 | FileCheck --check-prefix=GCN %s
; RUN: llc -amdgpu-late-wave-transform=1 < %s -mtriple=amdgpu8.02 | FileCheck --check-prefix=GCN %s
