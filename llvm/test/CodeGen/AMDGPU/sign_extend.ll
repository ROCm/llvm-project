...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu6.00-- -amdgpu-scalarize-global-loads=false < %s | FileCheck %s -allow-deprecated-dag-overlap -enable-var-scope --check-prefix=SI
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu8.02-- -amdgpu-scalarize-global-loads=false -mattr=-flat-for-global < %s | FileCheck %s -allow-deprecated-dag-overlap -enable-var-scope --check-prefix=VI
