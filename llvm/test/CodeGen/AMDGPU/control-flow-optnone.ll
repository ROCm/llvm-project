...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgcn-amd-amdhsa < %s | FileCheck -check-prefix=GCN %s

; optnone disables annotation of uniformity metadata in AMDGPUCodeGenPrepare, 
; so no branch is known to be uniform during instruction selection. The 
; custom selection for brcond was not checking if the branch was uniform, 
; relying on the selection pattern to check that. That would fail, so then 
; the branch would fail to select.
