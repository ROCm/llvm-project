...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel=0 -mtriple=amdgpu8.03-amd-amdhsa -o - -amdgpu-enable-lower-module-lds=false %s 2> %t | FileCheck -check-prefixes=GFX8,GFX8-SDAG %s
...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel=0 -mtriple=amdgpu9.00-amd-amdhsa -o - -amdgpu-enable-lower-module-lds=false %s 2> %t | FileCheck -check-prefixes=GFX9,GFX9-SDAG %s
; RUN: FileCheck -check-prefix=ERR %s < %t
...
; RUN: llc -amdgpu-late-wave-transform=0 -global-isel=0 -mtriple=amdgpu9.00-mesa-mesa3d < %s 2> %t | FileCheck -check-prefixes=CHECK,SDAG %s
; RUN: FileCheck -check-prefix=ERR %s < %t
