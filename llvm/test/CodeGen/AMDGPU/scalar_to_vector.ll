...
; RUN: llc -amdgpu-late-wave-transform=1 < %s -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu6.00 -mattr=-flat-for-global | FileCheck %s --check-prefix=SI
; RUN: llc -amdgpu-late-wave-transform=1 < %s -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu8.02 -mattr=-flat-for-global | FileCheck %s --check-prefixes=GFX89,VI
; RUN: llc -amdgpu-late-wave-transform=1 < %s -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu9.00 -mattr=-flat-for-global -| FileCheck %s --check-prefixes=GFX89,GFX9
; RUN: llc -amdgpu-late-wave-transform=1 < %s -amdgpu-scalarize-global-loads=false  -mtriple=amdgpu11.00 -mattr=-flat-for-global -| FileCheck %s --check-prefixes=GFX11
