; RUN: llc -amdgpu-late-wave-transform=1 -O0 -mtriple=amdgcn-amd-amdhsa -mcpu=gfx908 < %s | FileCheck -check-prefix=ALL %s
; RUN: llc -amdgpu-late-wave-transform=1 -O0 -mtriple=amdgcn-amd-amdhsa -mcpu=gfx90a < %s | FileCheck -check-prefixes=ALL,GFX90A %s
...
; ALL:       .set kernel.num_vgpr, max(32, .Laliasee_default.num_vgpr)
