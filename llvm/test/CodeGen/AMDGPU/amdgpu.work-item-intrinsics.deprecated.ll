; RUN: llc -mtriple=amdgpu6.00 < %s | FileCheck --check-prefixes=SI-NOHSA,GCN-NOHSA,FUNC %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -mtriple=amdgpu6.00 < %s | FileCheck --check-prefixes=SI-NOHSA,GCN-NOHSA,FUNC %s

; RUN: llc -mtriple=amdgpu8.02 -mattr=-flat-for-global < %s | FileCheck  --check-prefixes=VI-NOHSA,GCN-NOHSA,FUNC %s
; RUN: llc -amdgpu-late-wave-transform=1 -global-isel -mtriple=amdgpu8.02 -mattr=-flat-for-global < %s | FileCheck  --check-prefixes=VI-NOHSA,GCN-NOHSA,FUNC %s
