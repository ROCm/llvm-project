...
; RUN: llc < %s -show-mc-encoding -amdgpu-promote-alloca-to-vector-max-regs=32 -amdgpu-load-store-vectorizer=0 -enable-amdgpu-aa=0 -mtriple=amdgpu8.02-amdhsa -mattr=-unaligned-access-mode | FileCheck -enable-var-scope -check-prefix=SI-PROMOTE-VECT -check-prefix=SI -check-prefix=FUNC %s
