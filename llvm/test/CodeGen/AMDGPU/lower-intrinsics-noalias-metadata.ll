...
; XUN: opt -passes='amdgpu-lower-intrinsics,function(amdgpu-annotate-uniform)' -S -mtriple=amdgcn-amd-amdhsa -mcpu=gfx1100 -codegen-opt-level=1 -mattr=+wavefrontsize32 < %s | FileCheck --check-prefix=NOCLOBBER %s
...
; FIXME-WAVE-TRANSFORM: Disabled the first runline currently. The amdgpu-annotate-uniform pass is removed and the annotation is currently moved to the amdgpu-codegen-prepare.
