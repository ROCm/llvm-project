...
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.0a -stop-after=amdgpu-isel -amdgpu-atomic-optimizer-strategy=None < %s | FileCheck -check-prefixes=GFX90A_GFX942,GFX90A %s
; RUN: llc -amdgpu-late-wave-transform=1 -mtriple=amdgpu9.42 -stop-after=amdgpu-isel -amdgpu-atomic-optimizer-strategy=None < %s | FileCheck -check-prefixes=GFX90A_GFX942,GFX942 %s
...
define amdgpu_ps double @global_atomic_fadd_f64_rtn_atomicrmw(ptr addrspace(1) %ptr, double %data) {
  ...
  ; GFX90A-NEXT:   [[V_CMP_EQ_U64_e64_:%[0-9]+]]:sreg_64 = V_CMP_EQ_U64_e64 [[GLOBAL_ATOMIC_CMPSWAP_X2_RTN]], [[PHI]], implicit $exec
  ; GFX90A-NEXT:   [[COPY11:%[0-9]+]]:av_64_align2 = COPY [[GLOBAL_ATOMIC_CMPSWAP_X2_RTN]]
  ; GFX90A-NEXT:   SI_BRCOND %bb.2, killed [[V_CMP_EQ_U64_e64_]], implicit-def dead $exec, implicit-def dead $vcc, implicit $exec
