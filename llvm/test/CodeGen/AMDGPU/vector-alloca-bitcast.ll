...
; RUN: llc -mtriple=amdgpu8.02-- -amdgpu-promote-alloca-to-vector-max-regs=32 < %t.sroa.ll | FileCheck -enable-var-scope --check-prefixes=GCN,GCN-PROMOTE %s
; RUN: opt -S -mtriple=amdgpu-- -passes='sroa,amdgpu-promote-alloca,instcombine' -amdgpu-promote-alloca-to-vector-max-regs=32 < %s | FileCheck -check-prefix=OPT %s
