; RUN: llc -amdgpu-late-wave-transform=0 -enable-new-pm -mtriple=amdgcn--amdhsa -mcpu=gfx700 -O0 -print-pipeline-passes=tree < %s 2>&1 \
; RUN:   | FileCheck -check-prefix=GCN-O0 %s

; RUN: llc -amdgpu-late-wave-transform=0 -enable-new-pm -mtriple=amdgcn--amdhsa -mcpu=gfx700 -print-pipeline-passes=tree < %s 2>&1 \
; RUN:   | FileCheck -check-prefix=GCN-O2 %s

; RUN: llc -amdgpu-late-wave-transform=0 -O3 -enable-new-pm -mtriple=amdgcn--amdhsa -mcpu=gfx700 -print-pipeline-passes=tree < %s 2>&1 \
