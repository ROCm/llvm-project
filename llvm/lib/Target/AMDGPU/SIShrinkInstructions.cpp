...
class SIShrinkInstructions {
  ...
  bool shrinkScalarLogicOp(MachineInstr &MI, bool &MoveItr) const;
...
bool SIShrinkInstructions::shrinkScalarLogicOp(MachineInstr &MI, bool &MoveItr) const {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (I = MBB.begin(); I != MBB.end(); I = Next) {
      ...
      if (MI.getOpcode() == AMDGPU::S_AND_B32 ||
        ...
        bool MoveItr = false;
        Changed |= shrinkScalarLogicOp(MI, MoveItr);
        if (MoveItr)
          continue;
      }
