...
bool AMDGPUGlobalISelDivergenceLowering::runOnMachineFunction(
  ...
  if (EnableLateWaveTransform) {
    ...
    // outside cycle with divergent exit provided by uniformity analysis.
    // Uniform instructions from the list require lowering, no instruction is
    // deleted. Thus it needs to be run before lowerPhis that deletes phis that
    // require lowering and replaces them with new instructions.
    ...
    // Temporal divergence lowering of divergent i1 phi used outside of the
    // cycle could also be handled by lowerPhis but we do it in
    // lowerTempDivergenceI1 since in some case lowerPhis does unnecessary lane
    // mask merging.
