...
static bool salvageDbgVariableLocation(Instruction &I, DbgVariableRecord &DVR) {
  ...
  if (SalvagedExpr->holdsNewElements()) {
    while (SalvagedExpr && LocIt != LocationOps.end()) {
      ...
      Replacement = salvageNewDebugInfo(I, CurrentLocOps, AdditionalValues, Ops);
      if (!Replacement)
        break;
      SalvagedExpr = DIExpression::appendNewOpsToArg(
          SalvagedExpr, Ops, LocationIndex, Replacement->getType());
