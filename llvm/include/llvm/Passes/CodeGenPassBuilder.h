...
protected:
  ...
  bool addRegAllocPassFromOpt(PassManagerWrapper &PMW,
...
#define RA_PASS_WITH_PARAMS(NAME, CLASS, CREATE_PASS, PARSER, PARAMS)          \
  if (PB.checkParametrizedPassName(PassOpt, NAME)) {                           \
    ...
    addMachineFunctionPass(CREATE_PASS(Params.get()), PMW);                    \
    return true;                                                                 \
  }
#include "llvm/Passes/MachinePassRegistry.def"
    if (PassOpt != "default") {
...
void CodeGenPassBuilder<Derived, TargetMachineT>::addRegAllocPass(
  ...
  if (!addRegAllocPassFromOpt(PMW, RegAllocPassName)) {
