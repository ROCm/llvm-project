...
The client libraries themselves (device stubs, host dispatchers, and the
CMake package) live in the separate `Emissary repository
<https://github.com/ROCm/emissary>`_, which is also where the full design
documentation is maintained:

* ``docs/internal/llvm-support.md`` -- detailed walkthrough of the code
  listed above.
* ``docs/internal/emissary-architecture.md`` -- the client-library half.
* ``docs/internal/llvm-rfc.md`` -- draft RFC for upstreaming this mechanism.
* ``docs/client-guide.md`` -- how to write a new client library.
