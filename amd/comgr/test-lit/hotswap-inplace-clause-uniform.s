...
// COM: scope, provided the clause does not contain the hardware entrypoint's
// COM: first VMEM. HotSwap must retain an ordinary uniform clause while
// COM: removing an initial VMEM clause from older objects that predate the
// COM: native global_wb/v_nop entry workaround.
...
// A branch that exits the kernel cannot bypass the VMEM on the only path that
// reaches the clause, so the uniform clause remains valid.
...
// Both arms of this diamond execute a VMEM before reaching the join, so the
// prior-VMEM fact survives predecessor intersection.
