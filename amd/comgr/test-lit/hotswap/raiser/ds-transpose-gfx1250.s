...
; RUN: %hotswap_transpile_cli %t.hsaco --target-isa=gfx950 --emit-ir > %t.ll
...
; RUN: %hotswap_transpile_cli %t.hsaco --target-isa=gfx942 --emit-ir \
; RUN:   | %FileCheck %s
; RUN: %hotswap_transpile_cli %t.hsaco --target-isa=gfx1250 --emit-ir \
; RUN:   > %t.same.ll
...
; RUN: not %hotswap_transpile_cli %t.gfx13.hsaco --target-isa=gfx950 --emit-ir 2>&1 \
