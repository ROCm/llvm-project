...
; RUN: %hotswap_transpile_cli %t.hsaco --emit-ir=alloc_vgpr_kernel \
; RUN:   | %FileCheck %s --check-prefix=ALLOC
; RUN: %hotswap_transpile_cli %t.hsaco --emit-ir=sleep_var_kernel \
; RUN:   | %FileCheck %s --check-prefix=SLEEP

; RUN: not %hotswap_transpile_cli %t.hsaco --emit-ir=shader_cycles_kernel 2>&1 \
; RUN:   | %FileCheck %s --check-prefix=CYCLES
; RUN: not %hotswap_transpile_cli %t.hsaco --emit-ir=rtn32_kernel 2>&1 \
; RUN:   | %FileCheck %s --check-prefix=RTN32
; RUN: not %hotswap_transpile_cli %t.hsaco --emit-ir=rtn64_kernel 2>&1 \
