...
; RUN:     --emit-ir=workgroup_barrier_kernel 2>&1 \
; RUN:   | %FileCheck %s --check-prefix=SIGNAL-SPLIT-TARGET
; RUN: not %hotswap_transpile_cli %t.hsaco \
; RUN:     --emit-ir=wait_workgroup_kernel 2>&1 \
; RUN:   | %FileCheck %s --check-prefix=WAIT-SPLIT-TARGET
...
; Onto the GPU that never had the split barrier, the arrival raises to the
; whole barrier and so does the release, so the source pair raises to a pair.
; Neither is dropped: each says on its own that the workgroup meets here.
...
; WORKGROUP-NEXT: }
; Onto a target that splits the barrier itself, fusing the halves would say
; more than the source did where the target can say exactly what it said.
; SIGNAL-SPLIT-TARGET: unsupported-instruction-form: s_barrier_signal [SOP1]
; SIGNAL-SPLIT-TARGET-SAME: the target splits in two as well
	s_barrier_signal -1
	s_barrier_wait -1
	s_endpgm

	.globl	wait_workgroup_kernel
	.p2align	8
	.type	wait_workgroup_kernel,@function
wait_workgroup_kernel:
; WAIT-SPLIT-TARGET: unsupported-instruction-form: s_barrier_wait [SOPP]
; WAIT-SPLIT-TARGET-SAME: the target splits in two as well
	s_barrier_wait -1
	s_endpgm
...
; UNHANDLED-SOPP: unsupported-instruction-form: s_trap [SOPP]
	...
	.end_amdhsa_kernel
	.amdhsa_kernel wait_workgroup_kernel
		.amdhsa_kernarg_size 0
		.amdhsa_next_free_vgpr 1
		.amdhsa_next_free_sgpr 2
	.end_amdhsa_kernel
...
amdhsa.kernels:
  ...
  - .args: []
    ...
    .kernarg_segment_size: 0
    .max_flat_workgroup_size: 1024
    .name:           wait_workgroup_kernel
    .private_segment_fixed_size: 0
    .sgpr_count:     2
    .symbol:         wait_workgroup_kernel.kd
    .vgpr_count:     1
    .wavefront_size: 32
  - .args: []
    .group_segment_fixed_size: 0
