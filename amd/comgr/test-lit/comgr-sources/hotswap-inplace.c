//===- hotswap-inplace.c - In-place patch hotswap end-to-end test ---------===//
//
...
///
/// \file
/// End-to-end test binary for in-place patches: cluster_load -> global_load
/// mnemonic swap and s_clause -> s_nop byte overwrite.
///
/// Usage: hotswap-inplace <asm_file> <source_isa> <target_isa> [<output_elf>]
///
//===----------------------------------------------------------------------===//
...
int main(int argc, char *argv[]) {
  if (argc < 4)
    fail("usage: hotswap-inplace <asm_file> <source_isa> <target_isa> "
         "[<output_elf>]");
  ...
  amd_comgr_(set_data_name(AsmData, "test_inplace.s"));
