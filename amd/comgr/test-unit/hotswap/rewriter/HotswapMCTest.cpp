...
TEST(InitLLVM, ValidGfx1250) {
  ...
  EXPECT_LT(S.SWaitKmcntOpcode, S.MCII->getNumOpcodes());
...
TEST(CollectDirectBranchTargets,
     RejectsExternalPlainAddCallContinuationIntoReturnFunction) {
  LLVMState S = initLLVM(makeGfx1250Ident());
  ASSERT_TRUE(S.Valid);
  llvm::SmallVector<uint8_t> Bytes =
      assembleInstructions("s_endpgm\n"
...
TEST(CollectDirectBranchTargets, BoundsCanonicalSetPcReturn) {
  ...
  Dispatch.CallOffset = Decoded[5].Offset;
  Dispatch.SequenceStart = Decoded[3].Offset;
  Dispatch.SequenceEnd = Decoded[5].Offset;
  Dispatch.Targets.push_back(0);
  llvm::SmallVector<uint64_t, 1> ExternalEntries{0};
  Info = collectDirectBranchTargets(
      Decoded, S, /*TextAddr=*/0, /*TextSize=*/Bytes.size(), DeclaredEntries,
      FunctionRanges, ExternalEntries, /*Text=*/{}, {Dispatch});
  ASSERT_TRUE(Info);
  EXPECT_FALSE(Info->HasUnresolvedTargets);
  EXPECT_TRUE(Info->BoundedIndirectTransfers.contains(Decoded[1].Offset));
  EXPECT_TRUE(Info->BoundedIndirectTransfers.contains(Dispatch.CallOffset));
}
...
TEST(CollectDirectBranchTargets, RejectsMaterializedEntryIntoRelocationProof) {
  ...
  EXPECT_FALSE(Info->BoundedIndirectTransfers.contains(Dispatch.CallOffset));
}
...
makeRelocationDispatchTestObject(const LLVMState &S, bool DynamicIndex = false,
  ...
  if (WaitForLoad && !Append("s_wait_kmcnt 0"))
    return std::nullopt;
  ...
  std::memcpy(&TableSymbol, Object.Bytes.data() + Object.TableSymbolOffset,
              sizeof(TableSymbol));
  if (UseLiteral64Addend)
    TableSymbol.st_value += 0x30;
...
TEST(RelocationTableDispatch, ProvesCompleteConstantSlot) {
  ...
  EXPECT_TRUE(
      ControlFlow->BoundedIndirectTransfers.contains(Object.CallOffset));
  EXPECT_TRUE(ControlFlow->Targets.contains(Object.TargetOffset));

  // Linked SOP2 lit64 operands decode as absolute MC expressions rather than
  // immediates. They carry the same constant address proof.
  ObjectOr = makeRelocationDispatchTestObject(
      S, /*DynamicIndex=*/false, /*SlotOffset=*/0, /*WaitForLoad=*/true,
      /*UseLiteral64Addend=*/true);
  ASSERT_TRUE(ObjectOr);
...
TEST(RelocationTableDispatch, RejectsUnboundedOrInvalidSlot) {
  ...
  std::optional<RelocationDispatchTestObject> UnwaitedObjectOr =
      makeRelocationDispatchTestObject(S, /*DynamicIndex=*/false,
                                       /*SlotOffset=*/0,
                                       /*WaitForLoad=*/false);
  ASSERT_TRUE(UnwaitedObjectOr);
  llvm::Expected<std::vector<RelocationTableDispatch>> Dispatches =
      analyzeRelocationDispatchTestObject(*UnwaitedObjectOr, S);
  ASSERT_TRUE((bool)Dispatches) << llvm::toString(Dispatches.takeError());
  ...
  Dispatches = analyzeRelocationDispatchTestObject(*DynamicObjectOr, S);
  ASSERT_TRUE((bool)Dispatches) << llvm::toString(Dispatches.takeError());
  EXPECT_TRUE(Dispatches->empty());
