...
bool isAlreadyClusterMaskPatched(const PatchContext &Ctx, size_t Idx) {
  ...
  const InternalDecodedInst &Prev = Ctx.Decoded[Idx - 1];
  const MCInst &PI = Prev.Inst;
  auto OperandIsM0 = [&](unsigned OperandIdx) {
    if (OperandIdx >= PI.getNumOperands())
      return false;
    const MCOperand &Op = PI.getOperand(OperandIdx);
    return Op.isReg() && isM0Reg(MCRegister(Op.getReg()), MRI);
  };

  if (Prev.Mnemonic == "s_pack_hh_b32_b16") {
    if (PI.getNumOperands() < 3 || !OperandIsM0(0))
      return false;
    if (!PI.getOperand(1).isImm() || PI.getOperand(1).getImm() != 0)
      return false;
    return OperandIsM0(2);
  }
...
bool patchTensorLoadToLdsB0(PatchContext &Ctx, size_t Idx) {
  ...
  unsigned SkipToClear = SccLive ? 3 : 2;
  unsigned SkipClear = SccLive ? 3 : 2;
  if (!appendRequiredAsm(Ctx, Prefix, ReadClusterIdAsm, Context))
    return false;
  if (!appendRequiredAsm(Ctx, Prefix, "s_cmp_eq_u32 " + BaseSreg + ", 0",
                         Context))
    return false;
  if (!appendRequiredAsm(Ctx, Prefix,
                         "s_cbranch_scc1 " + std::to_string(SkipToClear),
                         Context))
    return false;
  if (!appendRequiredAsm(Ctx, Prefix, "s_mov_b32 " + BaseSreg + ", " + S,
                         Context))
    return false;
  ...
  if (!appendRequiredAsm(Ctx, Prefix, "s_branch " + std::to_string(SkipClear),
                         Context))
    return false;
  if (!appendRequiredAsm(Ctx, Prefix, "s_mov_b32 " + BaseSreg + ", " + S,
                         Context))
  ...
  if (!appendRequiredAsm(Ctx, Prefix,
                         "s_pack_hh_b32_b16 " + BaseSreg + ", 0, " + BaseSreg,
                         Context))
    return false;
