...
amd_comgr_status_t retargetCodeObject(const void *ElfData, size_t ElfSize,
  ...
  if (!Deferred.empty()) {
    if (!fixupTrampolineBranches(Deferred, Text, Elf.textSize(), LS)) {
      // A trampoline branch could not be encoded, so the working `Buf` copy
      // is already partially redirected. Fall back to the pristine input code
      // object rather than shipping a half-rewritten binary.
