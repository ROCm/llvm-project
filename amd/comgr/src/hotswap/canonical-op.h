...
// Architecture-neutral instruction identity used for dispatch in the
// raiser. Each entry maps to one or more LLVM AMDGPU MC opcodes via
// `OpcodeMap` (opcode-map.h). The grouping exists because the raiser
// dispatches on per-instruction *semantics*, while MC opcodes encode
// per-generation hardware identity: wave32/wave64 variants of the same
// op (`S_AND_SAVEEXEC_B32` / `S_AND_SAVEEXEC_B64`), cross-generation
// renames (`S_MAX_F32` / `S_MAX_NUM_F32`), and inline-immediate vs
// register-source twins all collapse to one CanonicalOp here so handler
// code stays free of TableGen-version-sensitive opcode literals.
//
// Relationship to LLVM lowering pseudos. The AMDGPU backend's lowering
// pseudos (`AMDGPU::*_PSEUDO`) are target-internal and stable only
// within a single LLVM revision; they are not part of any public
// header, so they are not a usable identity surface for an out-of-tree
// raiser. CanonicalOp is the durable substitute owned by hotswap.
//
// Adding a new CanonicalOp.
//   1. Add the enum value below in the appropriate format group
//      (SOPP / SMEM / SOPC / ... -- see the group comments). Pick a
//      name that mirrors the LLVM MC opcode's stable spelling when
//      one opcode dominates, or a descriptive group name when several
//      opcodes collapse here.
//   2. Add the `E(mc_opcode, CANONICAL_OP)` row(s) in
//      `opcode-map.cpp`'s table for every MC opcode that should
//      canonicalise to it.
//   3. Add the matching `S(CANONICAL_OP)` row in
//      `canonical-op.cpp::canonicalOpName` -- the switch is
//      `-Wswitch` covered against this enum, so the compiler will
//      flag the missing case if forgotten.
//
// Intentionally-missing instructions. The `Unknown` fallback at the
// top of the enum is the explicit "we have not classified this
// instruction" channel. The raiser surfaces `Unknown` as a structured
// `HotswapError` (`makeHotswapUnsupportedOpcodeError`), so unmapped
// instructions cannot silently miscompile. There is no
// "intentionally not added" marker -- if an instruction shows up in
// the corpus and we choose not to lift it, the handler returns a
// `HotswapError` from the dispatch path rather than getting a
// CanonicalOp entry. Comments next to specific enum values flag
// known-to-be-deliberately-limited support shapes (see e.g.
// `S_FF0_I32_B32` below for an example of a documented edge).
// clang-format off
enum class CanonicalOp : uint16_t {
  ...
  //     trade-off argument.
  //   * On gfx1250 (same-target): like every other wait counter the
  //     raiser relies on LLVM's memory model to re-emit the native
  //     wait from the IR's load/store ordering.  The async intrinsic's
  ...
  // gfx12+ scalar narrow loads: fetch 1 or 2 bytes from a uniform
  // address and zero/sign-extend into a 32-bit SGPR. Older ISAs have
  // no equivalent; a cross-target lift to a pre-gfx12 backend
  // necessarily lowers the narrow `load iN` to VMEM, which is
  // semantically correct but uniformity-lossy.
  ...
  // -1 when the source is all-ones. The SOPInstructions.td record
  // declares no ISel pattern, so the instruction is only emitted by
  // hand-written asm / inline-asm -- but the corpus contains it.
  ...
  // pair of `llvm.smin`/`llvm.smax` intrinsics. The backend's
  ...
  // back into v_mad_(nc|co|_i64_i32) on whichever target the raise
  // writes to -- identical to how V_MAD_U64_U32 lowers today.
  ...
  // -- SMEM atomics --
  // gfx8+ scalar-cache atomics. Lifted to `atomicrmw` IR; the SCOPE /
  // GLC bits fold into AtomicOrdering (monotonic) and whether the
  ...
  // through the same hand-rolled bpermute-based emulation because
  // gfx942 (the transpiler's target ISA) has neither isel pattern and
  // no in-tree pre-isel emulation.
  DS_LOAD_TR8_B64,
  ...
  // at the cost of a 64-dword stride granularity. Lifted jointly
  // with the non-ST64 variants by the DS handler's READ2/WRITE2
  // block; placed adjacent in the enum so the existing
  ...
  // -- MUBUF atomics --
  // Order is significant: the MUBUF handler dispatches via the range
  ...
  // The wave-size classifier flags these in the cross-wave case; the
  // MUBUF handler models them with raw-buffer atomics so same-wave
  ...
  // a no-op at the raiser (like every other wait counter): the
  // intrinsic's `IntrInaccessibleMemOrArgMemOnly` annotation prevents
  // reorder across the wait site, and the backend re-emits the native
...
// Stable human-readable identifier for a CanonicalOp (the enum's
// spelling, e.g. `"V_CMPX"` for `CanonicalOp::V_CMPX`). Used in
// diagnostics -- prefer this over `(int)sop` so errors name the
// instruction class rather than a raw enum position that drifts with
// enum edits.
