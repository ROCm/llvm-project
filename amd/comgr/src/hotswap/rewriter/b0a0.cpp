...
struct ControlFlowScanIndex {
  ...
  DenseMap<size_t, RelocationTableDispatch> RelocationDispatchesBySource;
...
static std::optional<ControlFlowScanIndex> buildControlFlowScanIndex(
  ...
  for (const RelocationTableDispatch &Dispatch : RelocationDispatches) {
    ArrayRef<InternalDecodedInst>::iterator Call =
        llvm::lower_bound(Decoded, Dispatch.CallOffset,
                          [](const InternalDecodedInst &DI, uint64_t Offset) {
                            return DI.Offset < Offset;
                          });
    if (Call == Decoded.end() || Call->Offset != Dispatch.CallOffset) {
    ...
    if (!ReturnRegister ||
      ...
      log() << "hotswap: relocation-table call at 0x"
            << utohexstr(Dispatch.CallOffset)
            << " has ambiguous dispatch metadata\n";
      return std::nullopt;
    }
    std::optional<uint64_t> Continuation = checkedAddUint64(
    ...
    for (uint64_t Target : Dispatch.Targets) {
      if (Target >= TextSize) {
        log() << "hotswap: relocation-table call at 0x"
              << utohexstr(Dispatch.CallOffset)
              << " has a target outside .text\n";
        return std::nullopt;
      }
      Index.Calls.push_back(
          {CallIndex, Target, *Continuation, *ReturnRegister});
      Index.DirectTargetsByTarget.push_back({CallIndex, Target});
      Index.TableCallableEntries.push_back(Target);
    }
  }
  llvm::sort(Index.TableCallableEntries);
  Index.TableCallableEntries.erase(
      std::unique(Index.TableCallableEntries.begin(),
                  Index.TableCallableEntries.end()),
      Index.TableCallableEntries.end());
  ...
  for (size_t ReturnPosition = 0; ReturnPosition != Index.SetPcIndices.size();
    ...
    for (size_t RangeIndex : CandidateRanges[ReturnPosition]) {
      ...
      const bool IsTableCallable =
          std::binary_search(Index.TableCallableEntries.begin(),
                             Index.TableCallableEntries.end(), FunctionBegin);
      if (Range.Symbol && Range.Symbol->getBinding() != ELF::STB_LOCAL &&
          !IsTableCallable)
        continue;
...
std::optional<DirectControlFlowInfo> collectDirectBranchTargets(
  ...
  for (;;) {
    ...
    Index = buildControlFlowScanIndex(Decoded, LS, TextAddr, *TextEnd,
                                      FunctionRanges, RelocationDispatches);
  ...
  for (size_t InstIndex : Index->BranchOrCallIndices) {
    ...
    if (!HasPcRelativeOperand) {
      ...
      DenseMap<size_t, RelocationTableDispatch>::const_iterator
          RelocationDispatch =
      ...
      if (RelocationDispatch != Index->RelocationDispatchesBySource.end()) {
        if (hasKnownControlFlowEntry(DeclaredEntries, EntryProofReturns,
          ...
          for (uint64_t TableTarget : RelocationDispatch->second.Targets)
          ...
          log() << "hotswap: resolved relocation-table call at 0x"
                ...
                << RelocationDispatch->second.Targets.size()
                << " finite target(s)\n";
          continue;
        }
      }
...
analyzeDirectControlFlow(const ElfView &Elf,
  ...
  return collectDirectBranchTargets(
      ...
      Elf.functionTextRanges(), DeclaredEntries->ExternalEntries, Text,
...
static std::optional<uint32_t> applyGfx1250B0toA0Rules(
  ...
  if (!DeclaredEntries)
    return std::nullopt;
  std::optional<DirectControlFlowInfo> ControlFlow = analyzeDirectControlFlow(
      Elf, Decoded, LS, ArrayRef<uint8_t>(Text, TextSize));
  if (!ControlFlow)
    return std::nullopt;
