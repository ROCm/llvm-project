...
Error handleSOPP(RaiseContext &Ctx, const DecodedInst &Di, OpResolver &Op) {
  ...
  // Two bits per source operand class select the bank of 256 VGPRs that class
  // indexes into. Latch the selecting low byte into the register file the
  // raiser holds rather than emitting anything.
