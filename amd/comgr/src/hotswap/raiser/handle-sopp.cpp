...
Error handleSOPP(RaiseContext &Ctx, const DecodedInst &Di, OpResolver &) {
  ...
  //
  // Sequentially consistent at agent scope is the weakest form that still
  // expands to a wait on every target. A workgroup-scope fence emits no vmcnt
  // wait at all on a GFX9 target, whose L1 already orders the workgroup, and
  // an `amdgpu-synchronize-as:local` fence emits no lgkmcnt wait, LDS being
  // totally ordered absent cross-address-space ordering. Both are right for a
  // fence that pairs with another thread's fence and wrong for a counter,
  // which pairs with nothing and only has to have retired.
