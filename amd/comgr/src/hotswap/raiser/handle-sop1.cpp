...
// Refuse Di as an instruction form the raiser does not lift, naming it by
// mnemonic, offset and encoding format. Detail says what about the form is not
// lifted.
static Error unsupported(RaiseContext &Ctx, const DecodedInst &Di,
                         const Twine &Detail = {}) {
...
static Error writeDst(RaiseContext &Ctx, const DecodedInst &Di, ParsedReg Dst,
                      Value *V, bool Is64) {
  assert(V->getType()->isIntegerTy(Is64 ? 64 : 32) &&
         "value width must be the width the opcode operates on");
  if (!Is64) {
    Ctx.registers().writeReg32(Dst, V);
    return Error::success();
  }
  if (Dst.RegKind == ParsedReg::EXEC && !waveMaskWidthIs(Ctx, 64))
