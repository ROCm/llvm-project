...
static Expected<unsigned> displacedSgpr(RaiseContext &Ctx,
  ...
  if (Idx + WidthInDwords > Ctx.registers().regFile().Sgpr.size())
    return refuseMovrel(
...
Error handleSOP1(RaiseContext &Ctx, const DecodedInst &Di, OpResolver &Op) {
  ...
  if (Di.CanonOp == CanonicalOp::S_MOVRELS_B32 ||
    ...
    if (Is64)
      Ctx.registers().writeReg64(*Dst, Regs.loadSGPR64(Ctx.B, *Src));
    else
      Ctx.registers().writeReg32(*Dst, Regs.loadSGPR32(Ctx.B, *Src));
    return Error::success();
  }
  ...
  if (Di.CanonOp == CanonicalOp::S_MOVRELSD_2_B32) {
    ...
    Ctx.registers().writeReg32(
        ParsedReg{ParsedReg::SGPR, *DstIdx, 1},
        Ctx.registers().regFile().loadSGPR32(Ctx.B, *Src));
    return Error::success();
  }
