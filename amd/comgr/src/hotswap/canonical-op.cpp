...
// match the layout of the `CanonicalOp` enum in `canonical-op.h` -- if
// a new CanonicalOp lands in the enum, the compiler's `-Wswitch`
// (enabled by default for scoped enums) flags the missing case here.
...
#undef S
  llvm_unreachable("unhandled CanonicalOp");
}
