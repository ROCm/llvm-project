...
// Same pipeline as compileMin(), but sets a differing, codegen-visible
// -mllvm option (-amdgpu-function-calls=true/false, alternating by thread
// index) before AMD_COMGR_ACTION_CODEGEN_BC_TO_RELOCATABLE. This exercises
// comgr's LLVM cl::opt option-fingerprint cache under concurrency via the
// plain C API: threads keep forcing the cache between two different active
// fingerprints, so if the cache under- or over-synchronizes, this should
// manifest as a hang, crash, or checkError() abort rather than silently
// passing.
int compileMinDifferingOptions(int Index) {
  ...
  const char *FunctionCallsFlag = (Index % 2) == 0
                                       ? "-amdgpu-function-calls=true"
                                       : "-amdgpu-function-calls=false";
  const char *CodeGenOptions[] = {"-O0", "-mllvm", FunctionCallsFlag};
  Status = amd_comgr_action_info_set_option_list(
      DataAction, CodeGenOptions,
...
int main(int argc, char *argv[]) {
  ...
  for (int I = 0; I < 30; I++)
    CompileThreads.push_back(std::thread(compileMinDifferingOptions, I));
