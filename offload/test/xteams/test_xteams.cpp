...
template <typename T> T *sim_dot(T *a, T *b, uint64_t array_size) {
  ...
  if (!d_status) {
    d_status = (uint32_t *)omp_target_alloc(
        sizeof(uint32_t) * (_XTEAM_NUM_TEAMS + 1), devid);
    ...
    omp_target_memset(d_status, 0, sizeof(uint32_t) * (_XTEAM_NUM_TEAMS + 1),
                      devid);
  }
...
#pragma omp target teams distribute parallel for num_teams(_XTEAM_NUM_TEAMS)   \
    ...
    for (uint64_t k = 0; k < _XTEAM_TOTAL_NUM_THREADS; k++) {
      T val0 = T(0);
      for (uint64_t i = 0; i < stride || ((k == _XTEAM_TOTAL_NUM_THREADS - 1) &&
...
// K2: redistribution
#pragma omp target teams distribute parallel for num_teams(_XTEAM_NUM_TEAMS)   \
    num_threads(_XTEAM_NUM_THREADS) is_device_ptr(d_scan_out)
    for (uint64_t k = 0; k < _XTEAM_TOTAL_NUM_THREADS; k++) {
